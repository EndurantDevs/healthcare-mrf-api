...
def _decoded_reference_payload(reference: object) -> bytes:
    if type(reference) is not str or not reference.startswith(BILLING_ENTITY_REF_PREFIX):
    ...
    if (
        len(encoded_payload) != _REFERENCE_PAYLOAD_CHARACTERS
        or any(
            character not in _URLSAFE_BASE64_CHARACTERS
            for character in encoded_payload
        )
    ):
        raise PTG2BillingAssociationDataError("billing entity reference is invalid")
    try:
