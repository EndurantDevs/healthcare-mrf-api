...
"""
_PROVIDER_MEMBERSHIP_INSERT_SQL = f"""
...
def _validate_provider_set_memberships(
    ...
    if set(npis_by_set) != set(provider_set_ids):
    ...
    for provider_set_id in provider_set_ids:
        ...
        if (
            not provider_npis
            or len(provider_npis) > MAX_PROVIDER_NPIS_PER_SET
...
async def _stage_provider_set_batch(
    ...
    *,
    ...
    """Stage and persist one bounded provider-set membership batch."""

    binding_ordinal, npis_by_set = await _bounded_provider_memberships(
        session, binding, provider_sets, state
    )
    await _persist_provider_membership_batch(
        session,
        projection_id,
        binding_ordinal,
        provider_sets,
...
async def _bounded_provider_memberships(
    ...
    _validate_provider_set_memberships(provider_set_ids, npis_by_set)
...
async def _persist_provider_membership_batch(
    session: Any,
    projection_id: str,
    binding_ordinal: int,
    provider_sets: list[dict[str, Any]],
    ...
    """Persist one validated membership batch and queue newly observed NPIs."""
    ...
    await session.execute(
        text(_PROVIDER_MEMBERSHIP_INSERT_SQL),
        {
