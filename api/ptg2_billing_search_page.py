...
"""Provider grouping and bounded price hydration for exact billing search."""
...
def validate_billing_search_sort_key(
    ...
    """Validate the frozen seven-member provider-address coordinate."""
...
def group_billing_geo_candidates(
    *,
    ...
    if type(binding_pin) is not BillingSearchBindingPin:
        raise serving_unavailable()
...
def _next_hydration_chunk(
    ...
    while (
        ...
        binding = candidate.binding_pin.binding
        scope = (binding.binding_ordinal, binding.snapshot_id)
        if (
...
def _partition_candidates(
    ...
    for candidate in candidates:
        ...
        if (
            scope_candidates
            and scope_candidates[0].binding_pin != candidate.binding_pin
        ):
            raise serving_unavailable()
...
async def _hydrate_partition(
    ...
    *,
    ...
    hydrated = await ptg2_billing_price_reader.hydrate_exact_billing_geo_prices(
        session,
        candidates[0].binding_pin.serving_tables,
    ...
    *,
    ...
    for partition in _partition_candidates(candidates):
        ...
        for candidate_index, price_witnesses in hydrated_by_candidate.items():
            ...
            matched_provider = BillingSearchMatchedProvider(candidate, price_witnesses)
...
def _admit_chunk_work(
    ...
    *,
    ...
    chunk_scopes = {
        (
            candidate.binding_pin.binding.binding_ordinal,
            candidate.binding_pin.binding.snapshot_id,
        )
...
async def hydrate_billing_search_page(
    ...
    *,
    ...
    if type(limit) is not int or not 1 <= limit <= BILLING_SEARCH_MAX_PAGE_SIZE:
        raise serving_unavailable()
    ...
    if (
        ...
        raise serving_unavailable()
    remaining_candidates = _candidate_window(retained_candidates, after_sort_key)
