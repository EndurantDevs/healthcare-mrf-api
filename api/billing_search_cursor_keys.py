...
def _document_key_entries(document: dict[str, object]) -> list[object] | None:
    if (
        frozenset(document) != _DOCUMENT_FIELDS
...
def _key_values(key_entry: object) -> tuple[str, bytes] | None:
...
def _keyring_from_environment(
    ...
    try:
        ...
        if type(document) is not dict:
            return None
        key_entries = _document_key_entries(document)
        if key_entries is None:
            return None
        keys_by_id: dict[str, bytes] = {}
        for key_entry in key_entries:
            key_values = _key_values(key_entry)
            if key_values is None or key_values[0] in keys_by_id:
                return None
            keys_by_id[key_values[0]] = key_values[1]
...
def load_billing_search_cursor_keyring(
    ...
    """Load one fail-closed cursor keyring from an injected environment."""
