...
BILLING_SEARCH_TRANSPORT_ISSUER = "healthporta-api-layer"
...
def _capabilities(value: object) -> tuple[str, ...]:
    if type(value) is not list or any(type(item) is not str for item in value):
        raise _fail()
    capability_values = tuple(value)
    if capability_values not in _CAPABILITY_SETS:
        raise _fail()
...
def verify_billing_search_post_transport(
    ...
    *,
    ...
    """Verify signed context and exact body bytes, retaining no body digest."""
