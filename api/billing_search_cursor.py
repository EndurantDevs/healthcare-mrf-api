...
class BillingSearchCursorKeyring:
    ...
    def __init__(
        ...
        *,
        ...
        key_ids = {key_id for key_id, _key in normalized_keys}
        key_values = {key for _key_id, key in normalized_keys}
        if (
            len(key_ids) != len(normalized_keys)
            or len(key_values) != len(normalized_keys)
            or normalized_active_id not in key_ids
        ):
            raise _fail()
        object.__setattr__(
            self,
            "_BillingSearchCursorKeyring__active_key_id",
            normalized_active_id,
        )
        object.__setattr__(
            self,
            "_BillingSearchCursorKeyring__keys",
            normalized_keys,
        )
...
def _cursor_parts(token: object) -> tuple[str, bytes]:
    if (
...
def _new_sealed_page_cursor(
    ...
    return _mint_billing_search_sealed_page_cursor(token, state)
...
def seal_billing_search_cursor(
    ...
    *,
    ...
    token = (
        f"{BILLING_SEARCH_CURSOR_PREFIX}_{key_id}_"
...
def _open_plaintext(
    ...
    if type(keyring) is not BillingSearchCursorKeyring:
        raise _fail()
    key_id, sealed_payload = _cursor_parts(token)
    try:
        return AESGCM(keyring.key_for(key_id)).decrypt(
...
def open_billing_search_cursor(
    ...
    *,
    ...
    state = _decoded_state(_open_plaintext(token, keyring))
    now = _canonical_timestamp(trusted_now)
    ...
    scope_matches = all(
        tuple(
            hmac.compare_digest(actual, _canonical_sha256(expected))
            for actual, expected in expected_scope_digests
        )
    )
    if not state.issued_at <= now < state.expires_at or not scope_matches:
        raise _fail()
