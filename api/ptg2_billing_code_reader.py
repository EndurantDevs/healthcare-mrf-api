...
class BillingCodeWitness:
    ...
    def __post_init__(self) -> None:
        normalized_system, normalized_code = _exact_code(self.code_system, self.code)
        if (
            _code_key(self.code_key) != self.code_key
            or normalized_system != self.code_system
            or normalized_code != self.code
        ):
            raise PTG2ManifestArtifactError(
                "PTG2 exact billing code witness is malformed"
            )
        for category, value in (
            ("negotiation arrangement", self.negotiation_arrangement),
            ("type version", self.billing_code_type_version),
            ("source name", self.source_name),
            ("source description", self.source_description),
        ):
            _optional_text(value, category=category)
...
def _billing_code_witness(
    ...
    *,
    ...
    return BillingCodeWitness(
        ...
        source_name=_optional_text(raw_row.get("source_name"), category="source name"),
