...
| <https://www.cms.gov/medicare/payment/prospective-payment-systems/acute-inpatient-pps/ms-drg-classifications-and-software> | CMS MS-DRG definitions manual and ICD-10-CM/PCS MS-DRG indexes | `ms-drg` | `MS_DRG` and observed `ICD10PCS` rows in `code_catalog`; MS-DRG to ICD-10-CM/PCS grouping links in `code_relationship` |
...
| AccessMRF, Payerset docs, MRF Data Solutions, TPAFS transparency-data, BCBS Association roster, TPA compliance pages/metadata indexes, CMS price-transparency-guide | decentralized payer Transparency in Coverage MRF source registry and schema references | `mrf-source-discovery` | searchable payer/source/plan/file URL catalog, including issuer and TPA-hosted sources, for PTG discovery and selected full PTG imports |
...
- MS-DRG to ICD-10-CM/PCS rows are grouping relationships in `code_relationship`, not equivalent procedure crosswalks. Do not use them to auto-expand CPT/HCPCS pricing searches.
