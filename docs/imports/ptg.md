...
- `HLTHPRT_PTG2_PUBLISH_DB_DEDUPE_FALLBACK=true` remains the manual fallback. Normal pre-COPY merged imports pass `db_dedupe_fallback=false` to publish so unique indexes validate final uniqueness without a DB `DISTINCT ON` rebuild. If `HLTHPRT_PTG2_RUST_PARSE_IN_WORKERS=true`, publish keeps the DB `DISTINCT ON` dedupe backstop even after pre-COPY merge.
...
- `HLTHPRT_PTG2_RUST_PARSE_IN_WORKERS=false` is the guarded worker-side parser path. When enabled, the producer transfers bounded raw negotiated-rate chunks to worker threads so `RateLite` construction, normalization, hashing, dedupe, and COPY row writing happen off the producer thread. It still honors `HLTHPRT_PTG2_RUST_SPLIT_NEGOTIATED_RATES` for giant `in_network` objects.
