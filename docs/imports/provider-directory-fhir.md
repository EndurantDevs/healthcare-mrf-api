...
and HTTP 423 are retried. HTTP 429 returns durable defer metadata; HTTP 410,
authentication and other client responses, and semantic failures in an HTTP
200 response stop immediately. Retry waits target one and two seconds of
cumulative sleeping, within a strict three-second wait budget. Request time is
not charged to that wait budget, so a slow request can still use the remaining
bounded attempts. A valid delta-seconds or HTTP-date `Retry-After` can extend a
wait only when it is no more than three seconds and still fits that budget. A
longer boundary stops inline retries and is returned as durable retry metadata
without truncation or an early request. This schedule has no environment or
CLI override. A recovered response clears prior transient diagnostics, while
an exhausted response records one final redacted diagnostic with aggregate
request elapsed time and the number of requests actually reissued.
...
python main.py start provider-directory-fhir \
  ...
  --source-id "$PROVIDER_DIRECTORY_SOURCE_ID" \
  --dataset-rehydrate-only \
  --rehydrate-dataset-id "$CURRENT_DATASET_ID" \
  --rehydrate-acquisition-root-run-id "$ACQUISITION_ROOT_RUN_ID" \
  --rehydrate-batch-size 5000
```

For a carrier-scale retained dataset, preserve the emitted
...
partition, linked-resource, reverse-lookup, effective Bulk, or stale-cleanup
path. A schedule-level Bulk request does not disable this REST pilot when
source-level selection has already rejected Bulk as ineligible; any source that
can actually enter Bulk remains serial. The foundation collections may overlap,
while the dependency-sensitive role collection remains a serial tail. Each
resource retains its own durable cursor, so a retry skips completed collections
and resumes only incomplete ones. This does not fan out pages within an opaque
cursor or scan archive rows in parallel. Progress and persisted resource
diagnostics report requested and effective resource-scan concurrency so a
fail-closed fallback to `1` remains visible.
