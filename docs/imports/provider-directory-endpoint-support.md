...
| Texas TMHP (`texas-tmhp`) | Probe-only | None | Probe | None configured | https://cmsinterop.tmhp.com/tmhp/fhir/pd/R4 | pdfhir_569c8c60329e96aefd3e5983 | Offset pagination is supported, but this campaign only probes. |
| Nebraska (`nebraska`) | Probe-only | None | Probe | None configured | https://dhhs-api.ne.gov/dhhs/trading-partner/api/cmsi/provider/1.0.0 | pdfhir_6843a6194d9793a0a6cc36b4 | Offset pagination is supported, but this campaign only probes. |
| UHC (`uhc`) | Probe-only | None | Probe | None configured | https://flex.optum.com/fhirpublic/R4 | pdfhir_0b5cfd565c53364a73981dcb | Search partitions and role reverse lookup exist, but this campaign only probes. |
...
| Maine (`maine`) | Probe-only | None | Probe | None configured | https://maineproviderdirectory.verityanalytics.org/fhir | pdfhir_59350503a7be3498a85d8cf2 | Probe-only Verity FHIR base; no resource acquisition profile. |
...
| Contra Costa (`contra-costa`) | Probe-only | None | Probe | None configured | https://ihyml0v6d9.execute-api.us-east-1.amazonaws.com/hxprod | pdfhir_8ee2865f928f1d67b8a86090 | Official catalog can return 403, so the fallback base is retained; probe-only. |
