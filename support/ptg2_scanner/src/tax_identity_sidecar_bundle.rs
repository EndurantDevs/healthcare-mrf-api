...
//!
//! This gate validates declared artifact metadata, verifies bytes against
//! caller-supplied SHA-256 digests, runs the fixed-memory v1/v2 pair validator,
//! and compares exact ordinal parity with a caller-supplied provider-group
//! universe. It does not authenticate descriptor provenance or the universe
//! source. It deliberately leaves v1 as the only projection authority.
//! Cross-row full-HMAC/type collision detection requires a separate bounded
//! external pass and is not claimed here.
...
const MAX_SHARD_ID_BYTES: usize = 1_024;
const MAX_BUNDLE_TEXT_BYTES: usize = 4_096;

const _: () = assert!(usize::BITS <= u64::BITS);

/// Validate and compare one v1/v2 pair against its exact group universe.
pub fn validate_tax_identity_sidecar_shard<U: ProviderGroupUniverse>(
    ...
    validate_tax_identity_sidecar_shard_with_progress(
        shard_id,
        v1,
        v2,
        universe,
        ignore_record_progress,
    )
}

/// Variant that reports every validated, digest-checked pair row for progress heartbeats.
...
>(
    ...
    let summary = {
        ...
        let Some(summary) = pair.validated_summary() else {
            return Err(invalid_data(UNIVERSE_MISMATCH));
        };
        summary
    };
...
pub fn finalize_tax_identity_sidecar_bundle(
    ...
    let bundle_digest = bundle_digest(&token_policy_id, shard_count, &shards)?;
...
fn validate_v1_metadata(
    ...
    if validate_token_policy_id(metadata.token_policy_id).is_err() {
        return Err(invalid_data(INVALID_METADATA));
    }
    let state_total = [
...
fn resolved_shard_id<'a>(
    ...
    if let Some(source) = source_shard_id {
        validate_shard_id(source)?;
    }
    if let Some(alias) = shard_id {
        validate_shard_id(alias)?;
    }
    if source_shard_id.is_some() && shard_id.is_some() && source_shard_id != shard_id {
        return Err(invalid_data(INVALID_METADATA));
    }
    Ok(source_shard_id.or(shard_id))
}

fn validate_shard_id(shard_id: &str) -> io::Result<()> {
    if shard_id.is_empty() || shard_id.len() > MAX_SHARD_ID_BYTES || shard_id.trim() != shard_id {
        return Err(invalid_data(INVALID_METADATA));
    }
    Ok(())
}

fn ignore_record_progress(_: u64) -> io::Result<()> {
    Ok(())
}

fn universe_mismatch(_: io::Error) -> io::Error {
    invalid_data(UNIVERSE_MISMATCH)
}
