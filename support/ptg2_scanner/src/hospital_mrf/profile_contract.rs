...
impl CmsProfile {
    ...
    fn parse_csv(version: &str) -> io::Result<Self> {
        match version {
            ...
            // Some producers label V3-shaped CSV files as 3, 3.0.1, or 4.0.0.
            // The existing V3 header and row checks still reject incompatible shapes.
            "3" | HOSPITAL_MRF_SCHEMA_VERSION | "3.0.1" | "4.0.0" => Ok(Self::V3),
            _ => Err(invalid(format!(
                "unsupported CMS CSV version {version:?}; expected 1, 1.0.0, 2, 2.0.0, 2.2.0, 2.2.1, 3.0.0, or producer-compatible 3, 3.0.1, or 4.0.0"
            ))),
        }
    }
}
