...
#[test]
fn producer_declared_csv_v3_aliases_require_v3_shape_and_preserve_version() {
    for version in ["3", "3.0.1"] {
        for format in [InputFormat::TallCsv, InputFormat::WideCsv] {
            ...
            assert!(!rows["payer_charge"].is_empty());
            let mrf = String::from_utf8(rows["mrf"].clone()).unwrap();
            assert_eq!(mrf.trim_end().split('\t').collect::<Vec<_>>()[3], version);

            let mut v2_records = csv_fixture_records(&fixture_v2_csv(format, "2.0.0"));
            let version_index = csv_fixture_index(&v2_records[0], "version");
            v2_records[1][version_index] = version.to_owned();
            assert_import_error(
                format,
                &csv_fixture_bytes(&v2_records),
                DEFAULT_MAX_FANOUT_ROWS,
        ...
        let mut json_payload: serde_json::Value = serde_json::from_slice(&fixture_json()).unwrap();
        json_payload["version"] = json!(version);
        assert_import_error(
            InputFormat::Json,
            &serde_json::to_vec(&json_payload).unwrap(),
            DEFAULT_MAX_FANOUT_ROWS,
    ...
    let version_index = csv_fixture_index(&unsupported[0], "version");
    unsupported[1][version_index] = "3.0".to_owned();
    assert_import_error(
        InputFormat::TallCsv,
