...
use super::contracts::{
    ...
    ProviderDirectoryProjectionCopyTimings, CANONICAL_ROW_CONTRACT_ID, COPY_COLUMN_CONTRACT_ID,
    COPY_CONTRACT_ID, COPY_SPOOL_CONTRACT_ID, COPY_SUMMARY_RECORD_KIND, MAX_COPY_STREAM_BYTES,
    MAX_COPY_SUMMARY_BYTES, PROVIDER_DIRECTORY_PROJECTION_COPY_MAGIC, TRANSFORM_CONTRACT_ID,
};
use super::pg_copy::encode_binary_copy;
use super::semantics::project_resource;
use crate::provider_directory_projection::contracts::{
    ProviderDirectoryInputFraming, PROVIDER_DIRECTORY_PROJECTION_DECODER_CONTRACT_ID,
    PROVIDER_DIRECTORY_PROJECTION_MAX_INPUT_BYTES,
    PROVIDER_DIRECTORY_PROJECTION_MAX_RESOURCE_BYTES,
};
use crate::provider_directory_projection::encode::decoded_resources;
use rayon::prelude::*;
...
fn canonical_input_proof(
    ...
    for (_row, payload) in projected {
        ...
        byte_count = byte_count
            ...
            .ok_or_else(|| invalid_data("provider-directory canonical input overflowed"))?;
    }
...
fn projection_summary(
    ...
    let resource_count = u64::try_from(prepared.rows.len())
        .map_err(|_| invalid_data("provider-directory resource count overflowed"))?;
    let first = prepared
        .rows
        .first()
...
fn encode_header(summary: &ProviderDirectoryProjectionCopySummary) -> io::Result<Vec<u8>> {
    let summary_bytes = serde_json::to_vec(summary).map_err(|error| {
        invalid_data(format!(
            "provider-directory COPY summary cannot be encoded: {error}"
        ))
    })?;
    ...
    let summary_length = u32::try_from(summary_bytes.len())
        .map_err(|_| invalid_data("provider-directory COPY summary length overflowed"))?;
...
pub fn decode_provider_directory_projection_copy_spool(
    ...
    let summary_end = prefix_bytes
        .checked_add(summary_length)
        .ok_or_else(|| invalid_data("provider-directory COPY summary overflowed"))?;
    let copy_length_end = summary_end
        .checked_add(8)
        .ok_or_else(|| invalid_data("provider-directory COPY length overflowed"))?;
    ...
    let copy_length = usize::try_from(declared_copy_length)
        .map_err(|_| invalid_data("provider-directory COPY length is invalid"))?;
    if copy_length == 0 || copy_length > MAX_COPY_STREAM_BYTES {
        return Err(invalid_data("provider-directory COPY length is invalid"));
    }
    let copy_end = copy_length_end
        .checked_add(copy_length)
        .ok_or_else(|| invalid_data("provider-directory COPY body overflowed"))?;
