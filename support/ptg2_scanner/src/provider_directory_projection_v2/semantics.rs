...
pub fn project_resource(
    ...
    let canonical_payload =
        serde_json::to_vec(&payload_json).expect("serde_json::Value serialization is infallible");
    ...
    let address_count = i32::try_from(addresses.len())
        .expect("a 32 MiB retained input cannot contain i32::MAX addresses");
    let relationship_count = i32::try_from(relationships.len())
        .expect("a 32 MiB retained input cannot contain i32::MAX relationships");
    let input_ordinal_u64 =
        u64::try_from(input_ordinal).expect("the 100,000-resource input bound fits u64");
...
mod coverage_tests {
    ...
    fn internal_semantic_validation_boundaries_fail_closed() {
        ...
        .is_err());
        assert!(
            typed_evidence("Location", &expanded(vec![geocode(json!(false))], vec![])).is_err()
        );
        assert!(typed_evidence(
            "Location",
