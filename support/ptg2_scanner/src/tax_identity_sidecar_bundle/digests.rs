use super::{
    ...
    TaxIdentitySidecarV2Metadata, MAX_BUNDLE_TEXT_BYTES,
    TAX_IDENTITY_SIDECAR_BUNDLE_CHECKPOINT_CONTRACT, TAX_IDENTITY_SIDECAR_COLLISION_AUDIT,
...
fn update_length_prefixed(hasher: &mut Sha256, value: &str) -> io::Result<()> {
    if value.len() > MAX_BUNDLE_TEXT_BYTES {
        return Err(invalid_data(INVALID_METADATA));
    }
