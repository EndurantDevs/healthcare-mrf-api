...
/// Normalized admission metadata for one additive v2 sidecar.
///
/// This is not the raw writer-event schema; an adapter must construct it from
/// authenticated source/run coordinates before bundle validation.
...
/// Ordered provider-group owners from the caller-designated membership artifact.
///
/// This trait does not authenticate the artifact or establish that it is the
/// complete release universe; those remain integration-time requirements.
