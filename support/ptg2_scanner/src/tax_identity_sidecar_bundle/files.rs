...
pub(super) fn reauthenticate_artifact(
    ...
    let (_, reopened_digest, reopened_identity) =
        open_authentic_artifact(path, expected_byte_count, expected_digest)?;
    if reopened_identity != *expected_identity || reopened_digest != expected_digest {
        return Err(invalid_data(ARTIFACT_DIGEST_MISMATCH));
    }
    Ok(())
}
...
pub(super) fn hash_open_file(file: &mut File, expected_byte_count: u64) -> io::Result<[u8; 32]> {
    file.seek(SeekFrom::Start(0)).map_err(unavailable_io)?;
    let mut hasher = Sha256::new();
    let mut buffer = [0u8; HASH_BUFFER_BYTES];
    let mut remaining = expected_byte_count;
    while remaining != 0 {
        let requested = remaining.min(HASH_BUFFER_BYTES as u64) as usize;
        let read = file
            .read(&mut buffer[..requested])
            .map_err(unavailable_io)?;
        if read == 0 {
            return Err(invalid_data(ARTIFACT_SIZE_MISMATCH));
        }
    ...
    if file.read(&mut trailing).map_err(unavailable_io)? != 0 {
        return Err(invalid_data(ARTIFACT_SIZE_MISMATCH));
    }
...
fn open_read_only_artifact(path: &Path) -> io::Result<File> {
    ...
    options.open(path).map_err(unavailable_io)
}

fn file_identity(file: &File) -> io::Result<FileIdentity> {
    let metadata = file.metadata().map_err(unavailable_io)?;
    #[cfg(unix)]
    {
...
fn unavailable_io(_: io::Error) -> io::Error {
    invalid_data(UNAVAILABLE_ARTIFACT)
}
