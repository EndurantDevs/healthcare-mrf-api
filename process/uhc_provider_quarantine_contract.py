...
def provider_quarantine_rejected_counts(
    ...
    for public_field, counter_field in (
        UHC_PROVIDER_QUARANTINE_REJECTED_COUNT_FIELDS.items()
    ):
        rejected_count_by_field[public_field] = _nonnegative_counter(
            counters,
            counter_field,
        )
    _validate_raw_counter_bounds(counters)
    _validate_rejected_count_balance(rejected_count_by_field)
    invalid_count = rejected_count_by_field[
        UHC_PROVIDER_QUARANTINE_REASON_INVALID_NPI_CHECKSUM
    ]
    return rejected_count_by_field if invalid_count else {}
...
def _validate_rejected_count_balance(
    rejected_count_by_field: Mapping[str, int],
) -> None:
    invalid_count = rejected_count_by_field[
        UHC_PROVIDER_QUARANTINE_REASON_INVALID_NPI_CHECKSUM
    ]
    individual_count = rejected_count_by_field[
        "invalid_npi_checksum_individual_records"
    ]
    facility_count = rejected_count_by_field[
        "invalid_npi_checksum_facility_records"
    ]
    address_count = rejected_count_by_field[
        "invalid_npi_checksum_address_rows"
    ]
    plan_count = rejected_count_by_field[
        "invalid_npi_checksum_provider_plan_rows"
    ]
    if (
    ...
    if invalid_count == 0 and any(rejected_count_by_field.values()):
        raise UhcProviderQuarantineError(
            "UHC provider quarantine counters do not balance"
        )
...
def validate_provider_quarantine_fact(
    ...
    *,
    ...
    if (
        ...
        != UHC_PROVIDER_QUARANTINE_CONTRACT_ID
        or quarantine_by_field.get("reason")
        != UHC_PROVIDER_QUARANTINE_REASON_INVALID_NPI_CHECKSUM
