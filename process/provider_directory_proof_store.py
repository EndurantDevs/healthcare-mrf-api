...
async def _locked_dataset_proof_lineage(
    ...
    try:
        selected_resources = _validated_string_list(
    ...
    if (
        resource_hash_contract == SEMANTIC_CONTENT_RESOURCE_HASH_CONTRACT
        and (not has_proof_resource_scope or proof_resource_scope is None)
    ):
        raise ProviderDirectoryProofStoreError(
            "provider directory proof parent resource scope is invalid"
        )
...
def _decoded_record(line: bytes) -> list[Any]:
    ...
    base_shape_valid = bool(
        isinstance(record, list)
        and len(record) in {7, 10}
        and all(isinstance(record[index], str) for index in range(4))
        and _HASH_RE.fullmatch(record[2]) is not None
        and not any(
    ...
    semantic_shape_valid = bool(
        ...
        and isinstance(record[8], str)
        and _HASH_RE.fullmatch(record[8]) is not None
        and isinstance(record[9], list)
        and record[9] == sorted(set(record[9]))
        and all(
            isinstance(name_hash, str)
            and _HASH_RE.fullmatch(name_hash) is not None
            for name_hash in record[9]
        )
    )
    if semantic_shape_valid:
        if record[0] == "Practitioner":
            semantic_shape_valid = record[2] == (
                composed_practitioner_semantic_sha256(
                    record[8],
                    record[9],
                )
            ...
            semantic_shape_valid = record[8] == record[2] and record[9] == []
    if not base_shape_valid or (len(record) == 10 and not semantic_shape_valid):
        raise ProviderDirectoryProofStoreError(
...
def _finalized_proof_record_group(
    ...
    distinct_records = {
        _stable_json(record): record for record in records
    }
    if len(distinct_records) == 1:
        return next(iter(distinct_records.values())), {
            "collision_identities": 0,
            "observation_variants": 0,
            "union_name_count": 0,
            "added_name_count": 0,
        }
    unique_records = list(distinct_records.values())
    if (
        any(len(record) != 10 for record in unique_records)
        or any(record[0] != "Practitioner" for record in unique_records)
        or len({record[7] for record in unique_records}) != 1
        or len({record[8] for record in unique_records}) != 1
        or len({_stable_json(record[3:7]) for record in unique_records}) != 1
    ):
        raise ProviderDirectoryProofStoreError(
    ...
    return merged_record, {
        ...
        "observation_variants": len(unique_records),
        "union_name_count": len(union_name_hashes),
        "added_name_count": len(union_name_hashes)
        - min(len(record[9]) for record in unique_records),
    }
...
def _merged_resource_proof_summary(
    ...
    def finalize_group() -> None:
        nonlocal resource_count, observed_contract
        if not current_records:
            return
        proof_record, group_diagnostics = _finalized_proof_record_group(
            current_records
        )
        record_contract = (
            proof_record[7]
            if len(proof_record) == 10
            else LEGACY_RESOURCE_HASH_CONTRACT
        )
        if observed_contract is None:
            observed_contract = record_contract
        elif observed_contract != record_contract:
            raise ProviderDirectoryProofStoreError(
        ...
        for diagnostic_name, diagnostic_value in group_diagnostics.items():
            semantic_union_diagnostics[diagnostic_name] += diagnostic_value
        resource_count += 1
    ...
    for line in heapq.merge(
        *(record_spool.lines(path) for path in record_spool.bounded_paths())
        ...
        current_records.append(proof_record)
    finalize_group()
    ...
    return _MergedResourceSummary(
        ...
        source_metrics_by_name=metrics_by_name,
        resource_hash_contract=(
            observed_contract or LEGACY_RESOURCE_HASH_CONTRACT
        ),
        semantic_union_diagnostics=semantic_union_diagnostics,
    )
    ...
    *,
    ...
    expected_proof_resource_scope = (
        sorted(
            {
                _clean_text(resource_type)
                for resource_type in proof_resource_scope
            }
        )
    ...
    if (
        ...
        or (
            expected_proof_resource_scope is not None
            and (
                not expected_proof_resource_scope
                or any(
                    not resource_type
                    for resource_type in expected_proof_resource_scope
                )
                or not set(expected_resources).issubset(
                    expected_proof_resource_scope
                )
            )
        )
    ...
    return _ProofLineage(
        ...
        proof_resource_scope=expected_proof_resource_scope,
    )
...
def _sealed_dataset_proof_metadata(
        ...
        **(
            {
                ...
                "semantic_union": dict(
                    sorted(
                        merged_proof.semantic_union_diagnostics.items()
                    )
                ),
            }
            if is_semantic_union
    ...
    return validate_stored_dataset_proof_metadata(
        ...
        expected_resource_hash_contract=merged_proof.resource_hash_contract,
...
def _validate_metadata_summary(
    ...
    semantic_contract = proof_by_field.get("contract_id") == (
        PROVIDER_DIRECTORY_SEMANTIC_CONTENT_PROOF_CONTRACT_ID
    )
    if semantic_contract != (lineage.proof_resource_scope is not None):
        raise ProviderDirectoryProofStoreError(
            "provider directory content proof contract changed"
        )
    semantic_union = proof_by_field.get("semantic_union")
    if semantic_contract:
        if (
            ...
            != proof_by_field.get(_SEMANTIC_PROJECTION_AS_OF_FIELD)
            or not isinstance(semantic_union, Mapping)
            or set(semantic_union)
            != {
                "added_name_count",
        ...
        for field_name, field_value in semantic_union.items():
...
def _assert_expected_proof_contract(
    ...
    expects_semantic_proof = expected_resource_hash_contract == (
        SEMANTIC_CONTENT_RESOURCE_HASH_CONTRACT
    )
    if is_semantic_proof != expects_semantic_proof:
        raise ProviderDirectoryProofStoreError(
            "provider directory content proof contract changed"
        )
    if expects_semantic_proof and proof_by_field.get(
        "resource_hash_contract"
    ) != SEMANTIC_CONTENT_RESOURCE_HASH_CONTRACT:
        raise ProviderDirectoryProofStoreError(
            "provider directory semantic proof contract changed"
        )
    if expects_semantic_proof:
