...
async def begin_publication(schema: str, import_id: str) -> str:
    ...
    async with db.transaction() as session:
        ...
        """),
                {"attempt": attempt, "import_id": str(import_id)},
            )
        ).scalar_one_or_none()
        if claimed is None:
...
async def capture_summary_inputs(session, schema: str) -> dict[str, int]:
    ...
    for name in SUMMARY_INPUTS:
        input_oid_by_table[name] = int(
            (
                await session.execute(text("SELECT to_regclass(:name)::oid"), {"name": qualified(schema, name)})
            ).scalar_one()
        )
...
async def complete_publication(
    ...
    generation,
    inputs,
    address_resolution_performed: bool,
) -> None:
    ...
    """),
            {
                "attempt": attempt,
                "generation": json.dumps(generation.as_dict()),
                "summary_oid": summary_oid,
                "inputs": json.dumps(inputs),
                "address_resolution_performed": address_resolution_performed,
                "address_content": json.dumps(address_content),
            },
        )
    ).scalar_one_or_none()
...
async def require_completed_publication(session, schema: str) -> dict:
    ...
    publication_receipt = (
        (await session.execute(text(f"SELECT * FROM {table} WHERE singleton"))).mappings().one_or_none()
    )
    if (
