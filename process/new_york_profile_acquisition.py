...
async def _fetch_response(session, destination, stage, request):
    ...
    _require(response.headers.get("Content-Type", "").split(";", 1)[0].strip().lower() == "application/json", "content_type_invalid")
    _require(response.headers.get("Content-Encoding", "identity").strip().lower() == "identity", "content_encoding_invalid")
