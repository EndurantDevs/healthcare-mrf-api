...
def _has_valid_profession_counts(counts_by_profession):
    return (isinstance(counts_by_profession, dict) and set(counts_by_profession) == set(PROFESSIONS)
...
def _has_valid_report(report):
    ...
    return (_is_sha256(report.get("content_sha256"))
            and type(report.get("content_bytes")) is int and 0 < report["content_bytes"] <= MAX_REPORT_BYTES
            and isinstance(file_name, str) and bool(file_name) and PurePath(file_name).name == file_name
            and file_name not in {".", "..", "snapshot.json", "manifest.json"}
            and all(isinstance(report.get(field), str) and report[field].strip()
...
class TennesseeProfileStore(SourceProfileStore):
    ...
    def _manifest(self, run_by_field):
        ...
        required_fields = {"control_run_id", "expected_current_run_id", "max_providers", "resume_from", "categories",
                    "report_professions", "snapshot_sha256", "snapshot_row_count", "source"}
        ...
        if (not _is_sha256(manifest["snapshot_sha256"]) or type(manifest["snapshot_row_count"]) is not int
        ...
        expected_source_by_field = {
            ...
            "agency": "Tennessee Department of Health", "source_url": SOURCE_URL,
            "coverage_scope": COVERAGE_SCOPE, "registry_generation": manifest["snapshot_sha256"],
        }
        descriptor = manifest["source"]
        if not isinstance(descriptor, dict) or any(descriptor.get(key) != expected for key, expected in expected_source_by_field.items()):
            raise ValueError("tennessee_profile_manifest_source_invalid")
        return manifest

    def _bundle_counts(self, run_by_field, artifacts):
        invalid_counts_by_field = {"invalid_bundle_artifacts": 1, "bundle_reports_sha256": None,
                   "bundle_source_records_by_profession": None, "bundle_facts_by_profession": None}
        if len(artifacts) != 1:
            return invalid_counts_by_field
        artifact = artifacts[0]
        bundle = artifact.get("metadata_json")
        manifest = self._manifest(run_by_field)
        if (artifact.get("source_key") != SOURCE_KEY or artifact.get("category") != "profile"
                or artifact.get("file_name") != "manifest.json" or not isinstance(bundle, dict)
                or bundle.get("schema_version") != SCHEMA_VERSION or bundle.get("run_id") != run_by_field["run_id"]
                or bundle.get("source_manifest") != manifest or not _is_sha256(bundle.get("reports_sha256"))):
            return invalid_counts_by_field
        canonical = encoded_json(bundle)
        if (artifact.get("artifact_id") != hashlib.sha256(encoded_json([run_by_field["run_id"], SOURCE_KEY])).hexdigest()
                or artifact.get("content_sha256") != hashlib.sha256(canonical).hexdigest()
                or type(artifact.get("content_bytes")) is not int or artifact["content_bytes"] != len(canonical)):
            return invalid_counts_by_field
        snapshot = bundle.get("snapshot")
        reports = bundle.get("reports")
        acquisition = bundle.get("acquisition")
        if (not isinstance(snapshot, dict) or snapshot.get("content_sha256") != manifest["snapshot_sha256"]
                or snapshot.get("file_name") != "snapshot.json" or type(snapshot.get("content_bytes")) is not int
                or snapshot["content_bytes"] <= 0 or not isinstance(reports, dict) or set(reports) != set(PROFESSIONS)
            ...
            return invalid_counts_by_field
        report_pins_by_profession = {profession: {
            "content_sha256": report["content_sha256"], "evidence": {
                "run_id": run_by_field["run_id"], "artifact_id": artifact["artifact_id"],
                **{field: report[field] for field in ("source_url", "downloaded_at", "content_sha256")},
            },
        } for profession, report in reports.items()}
        if hashlib.sha256(encoded_json(report_pins_by_profession)).hexdigest() != bundle["reports_sha256"]:
            return invalid_counts_by_field
        return {"invalid_bundle_artifacts": 0, "bundle_reports_sha256": bundle["reports_sha256"],
    ...
    async def retained_counts(self, run_id):
        ...
        retained = await shared_store.db.first(text(f"""
            SELECT
        ...
        counts["source_records_by_profession"] = {"1606": counts["md_source_records"], "1907": counts["do_source_records"]}
        counts["facts_by_profession"] = {"1606": counts["md_facts"], "1907": counts["do_facts"]}
        artifact_rows = await shared_store.db.all(select(shared_store.ProviderProfileArtifact.__table__).where(
            shared_store.ProviderProfileArtifact.__table__.c.run_id == run_id))
        counts["retained_artifacts"] = len(artifact_rows)
        counts.update(self._bundle_counts(await self._read_run(run_id), [dict(artifact._mapping) for artifact in artifact_rows]))
        return counts
    ...
    async def _assert_source_ownership(self, run_ids):
        for run_id in run_ids:
            ...
            if not any(counts[name] for name in ("retained_source_records", "retained_facts", "retained_artifacts")):
                continue
            if (counts["received_profiles"] != counts["retained_source_records"] or any(counts[name] for name in (
    ...
    def _completion_metrics(self, run_by_field, metrics, counts_by_field):
        ...
        if (type(metrics.get("http_responses")) is not int or metrics["http_responses"] != 8
                or type(metrics.get("report_responses")) is not int or metrics["report_responses"] != 2
                or type(metrics.get("response_bytes")) is not int or metrics["response_bytes"] <= 0):
            raise RuntimeError("tennessee_profile_response_count_mismatch")
        if (not _is_sha256(metrics.get("reports_sha256")) or metrics["reports_sha256"] != counts_by_field["bundle_reports_sha256"]
        ...
        for name in ("source_records_by_profession", "facts_by_profession"):
            if (not _has_valid_profession_counts(metrics.get(name)) or metrics[name] != counts_by_field[name]
                    or metrics[name] != counts_by_field["bundle_" + name]):
                raise RuntimeError("tennessee_profile_retained_count_mismatch")
        if (sum(counts_by_field["source_records_by_profession"].values()) != counts_by_field["retained_source_records"]
                ...
                or any(counts_by_field[key] for key in (
                    "invalid_source_records", "invalid_facts", "foreign_artifacts", "invalid_bundle_artifacts",
                    "invalid_bundle_records", "invalid_report_facts",
                ))):
    ...
    def _publication_volume(self, metrics, incumbent_metrics):
        if incumbent_metrics is None:
            if (metrics["md_source_records"] < 60000 or metrics["do_source_records"] < 6000
...
class TennesseeProfileCompletion(SourceProfileCompletion):
    ...
    def _terminal_progress(self, result):
        ...
        return {"unit": "record", "done": completed, "total": completed, "pct": 100,
                "phase": f"{self.importer} published", "message": "succeeded"}


store = TennesseeProfileStore(ProfileSourcePolicy(
    source_key=SOURCE_KEY, schema_version=SCHEMA_VERSION, jurisdiction="TN", categories=CATEGORIES,
