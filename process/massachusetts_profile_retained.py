...
def _parent_manifest(directory, parent, artifact):
    ...
    if (artifact["source_key"] != SOURCE_KEY or artifact["run_id"] != parent["run_id"]
            or artifact["file_name"] != "manifest.json" or artifact["category"] != "profile"
            or manifest.get("schema_version") != SCHEMA_VERSION or manifest.get("run_id") != parent["run_id"]
    ...
    required_fields = {"responses", "response_bytes", "reused_responses", "responses_sha256", "acquisition_complete", "transport_failures"}
    if (not isinstance(metrics, dict) or not required_fields <= metrics.keys()
            or any(parent["metrics"].get(key) != value for key, value in metrics.items())
            or metrics["acquisition_complete"] is not True or metrics["transport_failures"] != 0):
        raise ValueError("massachusetts_profile_retained_metrics_invalid")
    if (type(metrics["response_bytes"]) is not int or metrics["response_bytes"] < 0
            or type(metrics["reused_responses"]) is not int or not 0 <= metrics["reused_responses"] <= metrics["responses"]):
        raise ValueError("massachusetts_profile_retained_metrics_invalid")
    return metrics
...
async def validate_acquisition(directory, parent, artifact, progress):
    ...
    if profiles.is_symlink() or {entry.name for entry in profiles.iterdir()} != {f"{root['license_number']}.json" for root in roots}:
    ...
    for index, root in enumerate(roots):
        ...
        response_hash.update(acquisition.encoded_json([license_number, response["content_sha256"], response["downloaded_at"]]))
    ...
    if (total_bytes != metrics["response_bytes"] or response_hash.hexdigest() != metrics["responses_sha256"]
            or ("response_envelopes_sha256" in metrics
    ...
    return cohort, {
        "lineage": {"source_run_id": parent["run_id"], "artifact_id": artifact["artifact_id"],
                    "manifest_sha256": artifact["content_sha256"], "response_envelopes_sha256": envelope_digest.hexdigest()},
...
async def copy_profiles(roots, retained, destination, hashes_by_license, progress):
    ...
    for index, root in enumerate(roots):
        await progress(index, len(roots))
        license_number = root["license_number"]
        response = checked_response(retained / f"{license_number}.json", license_number, hashes_by_license[license_number])
        total_bytes += len(response["body_text"].encode("utf-8"))
        if total_bytes > acquisition.MAX_ACQUISITION_BYTES:
            raise ValueError("massachusetts_profile_acquisition_too_large")
        ...
        envelope_digest.update(acquisition.encoded_json([license_number, hashes_by_license[license_number]]))
    await progress(len(roots), len(roots))
    return {"responses": len(roots), "response_bytes": total_bytes, "reused_responses": len(roots),
            "responses_sha256": response_hash.hexdigest(), "response_envelopes_sha256": envelope_digest.hexdigest()}
