...
async def _configured_crawl_targets(
    ...
    async_resolver = _ASYNC_CRAWL_RESOLVERS.get(resolver_type)
    if async_resolver:
        return await async_resolver(source_row, url, resolver_by_key, session)
    sync_resolver = _SYNC_CRAWL_RESOLVERS.get(resolver_type)
    if sync_resolver:
