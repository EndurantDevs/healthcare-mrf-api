...
def classify_hosting_platform(url: str | None) -> str | None:
    ...
    if host == "healthy.kaiserpermanente.org" and "machine-readable" in path:
        if path == "/hawaii/front-door/machine-readable":
            return "kaiser_hawaii_mrf_inventory"
        return "kaiser_mrf_inventory"
    if (
    ...
    if (
        host in {"uhealthplan.utah.edu", "www.uhealthplan.utah.edu"}
        and "machine-readable-data" in path
    ):
        return "html_mrf_links"
    if (
    ...
    if session is None:
        ...
        async with aiohttp.ClientSession(
            ...
            return await _post_json_value(
                url, request_payload, max_bytes=max_bytes, session=owned_session
            )
    async with session.post(
        url, json=request_payload, allow_redirects=True, **_request_ssl_kwargs(url)
    ) as resp:
        await _assert_fetch_url_allowed(str(resp.url))
...
def _candidate_metadata(
    ...
    for key, values in optional_values_by_field.items():
        cleaned_values = [
            _clean_text(value) for value in values if _clean_text(value)
        ]
        if cleaned_values:
            metadata[key] = list(dict.fromkeys(cleaned_values))
    return metadata
...
async def _store_observations(
    ...
    *,
    ...
    source_url_pairs = [
        (source_row, source_row.get("index_url") or source_row.get("human_url"))
        for source_row in source_rows
        if source_row.get("index_url") or source_row.get("human_url")
    ]
    total = len(source_url_pairs)
...
def _uhc_provider_mrf_targets_from_payload(
    source_row: dict[str, Any],
    provider_file_payload: dict[str, Any],
    *,
    listing_url: str,
    ...
    for section in ("providers", "drugs", "plans"):
        ...
        entries = provider_file_payload.get(section) if isinstance(provider_file_payload, dict) else None
...
def _mymedicalshopper_employer_search_values(employer: dict[str, Any]) -> list[Any]:
    return [
        employer.get("name"),
        employer.get("slug"),
        employer.get("groupId"),
        employer.get("group_id"),
        employer.get("ein"),
        employer.get("tpaName"),
        employer.get("tpa_name"),
    ]
...
def _mymedicalshopper_query_fallback_max_employers(
    resolver: dict[str, Any],
    max_employers: int,
) -> int:
    configured = _as_int(resolver.get("query_search_fallback_max_employers"))
    fallback_max = configured if configured and configured > 0 else 500
    return max(1, min(max_employers, fallback_max))


@dataclass(frozen=True)
class _MmsEmployerQuery:
    entity_slug: str
...
async def _mymedicalshopper_entity_employer_page(
    ...
    info_messages = await _mymedicalshopper_ddp_subscribe_collect(
        ...
        sub_id=(
            "mms-info-"
            f"{employer_query.entity_slug}-{employer_query.selector_label}-{offset}"
        ),
        timeout_seconds=employer_query.timeout_seconds,
    )
    table_info = _mymedicalshopper_tabular_info_from_messages(info_messages)
    employer_ids = [
        employer_id for employer_id in table_info.get("ids") or [] if employer_id
    ]
    if not employer_ids:
        return table_info, [], 0
    doc_messages = await _mymedicalshopper_ddp_subscribe_collect(
        ...
        params=["EntityMRFEmployers", employer_ids, employer_query.fields],
        sub_id=(
            "mms-employers-"
            f"{employer_query.entity_slug}-{employer_query.selector_label}-{offset}"
        ),
    ...
    return (
        table_info,
        _mymedicalshopper_employer_docs_from_messages(doc_messages),
        len(employer_ids),
    )


async def _mymedicalshopper_entity_employers_for_query(
    ws: aiohttp.ClientWebSocketResponse,
    employer_query: _MmsEmployerQuery,
    ...
    offset = 0
    while (
        offset < employer_query.max_employers
        and len(employers_by_slug) < employer_query.max_employers
    ):
        batch_size = min(
            employer_query.page_size,
            employer_query.max_employers - len(employers_by_slug),
        )
        table_info, employer_docs, employer_id_count = (
            await _mymedicalshopper_entity_employer_page(
                ws, employer_query, offset, batch_size
            )
        )
        if not employer_docs:
            break
        for employer in employer_docs:
            if (
                employer_query.query_filter
                and not _has_mymedicalshopper_employer_query_match(
                    employer, employer_query.query_filter
                )
            ):
                continue
            slug = str(employer.get("slug") or "").strip()
            if slug:
                employer_payload_dict = dict(employer)
                employer_payload_dict.setdefault("tpaSlug", employer_query.entity_slug)
                if employer_query.tpa_name:
                    employer_payload_dict.setdefault(
                        "tpaName", employer_query.tpa_name
                    )
                employers_by_slug[slug] = employer_payload_dict
        offset += employer_id_count
        if offset >= (
            _as_int(table_info.get("records_filtered")) or employer_id_count
        ):
            break
...
async def _mymedicalshopper_employer_discovery_context(
    ...
    search_selector = _mymedicalshopper_employer_search_selector(
        base_selector, target_query
    )
    selectors = _mymedicalshopper_entity_employer_selectors(
        entity_slug,
        all_employers_searchable=all_employers_searchable,
...
async def _resolve_viva_health_mrf(
    ...
    if resolver.get("include_employer_mrf_links", True):
        ...
        try:
            ...
            employer_targets = []
        crawl_targets.extend(employer_targets)
...
def _sapphire_toc_target(
    ...
    *,
    ...
    raw_url_text = html.unescape(str(raw_url or "")).strip()
    if not raw_url_text:
        return None
    url = urljoin(base_url or "", raw_url_text)
    ...
    if inferred_file_name:
        target_by_field["file_name"] = inferred_file_name
    if inferred_payer_name:
        target_by_field["payer_name"] = inferred_payer_name
...
async def _resolve_sapphire_static_query_toc_links(
    ...
    for target_by_field in _parse_sapphire_static_query_toc_links(
        page_data_text, base_url=url
    ):
        key = _canonical_or_none(str(target_by_field.get("url") or "")) or str(
            target_by_field.get("url") or ""
        )
        if key:
            targets_by_url[key] = target_by_field
    ...
    for query_hash in _sapphire_static_query_hashes(page_data_text)[:max_queries]:
        ...
        for target_by_field in _parse_sapphire_static_query_toc_links(
            query_text, base_url=url
        ):
            key = _canonical_or_none(
                str(target_by_field.get("url") or "")
            ) or str(
                target_by_field.get("url") or ""
            )
            if key:
...
async def _resolve_anthem_s3_mrf(
    ...
    context_targets = await _resolve_anthem_s3_context_files(
        source_row, resolver, patterns, bases[0], url, session
    )
    if context_targets is not None:
        return context_targets
    crawl_targets = _anthem_s3_toc_targets(
        source_row,
        bases[0],
        patterns,
        resolver,
...
def _wordpress_elfinder_targets_from_payload(
    ...
    *,
    ...
    crawl_targets: list[CrawlTarget] = []
    for file_entry in _wordpress_elfinder_files(directory_payload):
        if str(file_entry.get("mime") or "").lower() == "directory":
            continue
        ...
        crawl_targets.append(
            CrawlTarget(
                ...
                metadata={
                    key: entry_value
                    for key, entry_value in metadata.items()
...
def _webtpa_record_target(
    ...
    *,
    ...
    file_name = _clean_text(
        file_record.get("fileName") or file_record.get("name")
    )
    label = " - ".join(part for part in (plan_name, file_name) if part)
    plan_info_rows: list[dict[str, Any]] = []
    ...
    return CrawlTarget(
        ...
        metadata={
            ...
            "webtpa_record_type": file_record.get("type"),
        },
    )
...
def _cmstic_brand_candidates_from_url(url: str, resolver: dict[str, Any]) -> list[str]:
    ...
    if isinstance(brands, list):
        ...
        for item in brands:
            ...
            if item and item not in seen_brands:
                seen_brands.add(item)
                deduped_brands.append(item)
        if deduped_brands:
            return deduped_brands
...
def _strict_anthem_context_ein(
    source_row: dict[str, Any],
    configured_ein: str,
) -> str:
    """Return the trusted EIN when the private context requests exact lookup."""
    lookup_type = re.sub(
        r"[^a-z0-9]+",
        "_",
        _source_query_context_text(
    ...
    ein_digits = "".join(
        character for character in configured_ein if character.isdigit()
    )
    if lookup_type == "employer_ein" and len(ein_digits) == 9:
        return ein_digits
    return ""


def _anthem_name_matches_for_context(
    source_row: dict[str, Any],
    configured_ein: str,
    name_matches: Iterable[dict[str, str]],
) -> tuple[str, list[dict[str, str]]]:
    """Restrict exact-EIN contexts while preserving normal name expansion."""
    strict_ein = _strict_anthem_context_ein(source_row, configured_ein)
    eligible_matches = list(name_matches)
    if strict_ein:
        eligible_matches = [
            name_match
            for name_match in eligible_matches
            if name_match["ein"] == strict_ein
        ]
    return strict_ein, eligible_matches


def _anthem_candidate_matches(
    *,
    direct_targets: list[CrawlTarget],
    employer_ein: str,
    strict_ein: str,
    name_matches: Iterable[dict[str, str]],
) -> tuple[dict[str, dict[str, str]], set[str]]:
    """Validate resolved employer candidates before fetching their file lists."""
    candidates_by_ein = {
        name_match["ein"]: name_match for name_match in name_matches
    }
    indexed_eins = set(candidates_by_ein)
    if candidates_by_ein or direct_targets:
        return candidates_by_ein, indexed_eins
    if strict_ein:
        raise ValueError(
            "no current Anthem employer MRF result for configured EIN "
            f"{employer_ein}"
        )
    if employer_ein:
...
async def _resolve_anthem_candidate_targets(
    source_row: dict[str, Any],
    candidate_matches: Iterable[dict[str, str]],
    indexed_eins: set[str],
    lookup_context: AnthemLookupContext,
) -> list[CrawlTarget]:
    """Fetch every name-index candidate without accepting partial results."""
    resolved_targets: list[CrawlTarget] = []
    for candidate_match in candidate_matches:
        try:
            candidate_targets = await _resolve_anthem_s3_employer_files(
                source_row,
                employer_ein=candidate_match["ein"],
                lookup_context=lookup_context,
                matched_employer_name=candidate_match.get("name"),
                name_index_url=candidate_match.get("name_index_url"),
            )
        except ValueError as exc:
            if candidate_match["ein"] in indexed_eins:
                raise ValueError(
...
async def _resolve_anthem_s3_context(
    ...
    *,
    ...
    try:
        ...
        raise
    strict_ein, name_matches = _anthem_name_matches_for_context(
        source_row,
        employer_ein,
        name_matches,
    )
    candidates_by_ein, indexed_eins = _anthem_candidate_matches(
        direct_targets=direct_targets,
        employer_ein=employer_ein,
        strict_ein=strict_ein,
    ...
    resolved_targets.extend(
        await _resolve_anthem_candidate_targets(
            source_row,
            candidates_by_ein.values(),
            indexed_eins,
            lookup_context,
        )
    )
...
def _plan_rows_from_target_metadata(crawl_target: CrawlTarget) -> list[dict[str, Any]]:
...
def _bcbsnc_aso_crawl_target(
    ...
    toc_url = _clean_text(result_metadata.get("url"))
    toc_crawl_target = _direct_toc_crawl_target(
        source_row,
        toc_url,
    ...
    source_context = _source_query_context_metadata(source_row)
    employer_name = (
        _clean_text(result_metadata.get("groupname"))
        or _clean_text(source_context.get("employer_name"))
    ...
    return CrawlTarget(
        ...
        metadata={
            ...
            **source_context,
            ...
            "bcbsnc_search_result_id": _clean_text(matched_search_result.get("id")),
...
async def _resolve_bcbs_asomrf_filelist(
    ...
    for filelist_url in filelist_urls:
        file_list_payload = await _fetch_json_value(
            filelist_url,
...
def _kaiser_inventory_months(
    resolver: dict[str, Any], *, now: dt.datetime | None = None
) -> list[str]:
    month_offsets = resolver.get("month_offsets")
    if not isinstance(month_offsets, list) or not month_offsets:
        month_offsets = [0, -1]
    ...
    for raw_offset in month_offsets:
        ...
        if offset is None:
            continue
        month = _add_months(current, offset).strftime("%Y-%m")
        if month not in months:
            months.append(month)
    return months


async def _resolve_kaiser_monthly_inventory(
    source_record: dict[str, Any],
    url: str,
    resolver: dict[str, Any],
    session: aiohttp.ClientSession,
) -> list[CrawlTarget]:
    """Resolve Kaiser's current regional in-network and allowed inventories."""
    base_url = str(
        resolver.get("base_url")
        or "https://healthy.kaiserpermanente.org/pricing/"
    ).rstrip("/")
    inventory_max_bytes = int(
        resolver.get("inventory_max_bytes") or 2 * 1024 * 1024
    )
    crawl_targets: list[CrawlTarget] = []
    for category in ("innetwork", "outofnetwork"):
        for inventory_month in _kaiser_inventory_months(resolver):
            inventory_url = (
                f"{base_url}/{category}/{inventory_month}_List.txt"
            )
            inventory_text = await _fetch_text(
                inventory_url,
                max_bytes=inventory_max_bytes,
                session=session,
            )
            category_targets = _kaiser_inventory_targets_from_text(
                source_record,
                inventory_text,
                inventory_url=inventory_url,
                inventory_month=inventory_month,
            ...
            if category_targets:
                crawl_targets.extend(category_targets)
                break
    crawl_targets = _dedupe_crawl_targets_by_url(crawl_targets)
    max_targets = _as_int(resolver.get("max_targets"))
    crawl_targets = _limit_crawl_targets_round_robin(
        crawl_targets, "kaiser_region_code", max_targets
    )
    if not crawl_targets:
        raise ValueError(f"no Kaiser MRF inventory targets found for {url}")
    return crawl_targets
...
def _providence_toc_targets_from_payload(
    ...
    *,
    ...
    crawl_targets: list[CrawlTarget] = []
    for group in payload_by_field.get("groups") or []:
        if not isinstance(group, dict):
            continue
...
async def _resolve_crawl_targets(
    ...
    *,
    ...
    source_items = [
        (idx, source_row, source_row.get("index_url") or source_row.get("human_url"))
    ...
    return {
        "mrf_file_id": _id(
            "mrffile",
            {
                "source": source_row["source_id"],
                "type": file_type,
                "url": _canonical_or_none(crawl_target.url),
            },
        ),
        "payer_id": source_row.get("payer_id"),
        "source_id": source_row["source_id"],
        "file_type": file_type,
        "url": crawl_target.url,
        "canonical_url": _canonical_or_none(crawl_target.url),
        ...
        **_toc_target_plan_fields(target_metadata_by_field),
        ...
        "metadata_json": {
            ...
            **_file_benefit_metadata(
                ...
                crawl_target.url,
                crawl_target.label,
                crawl_target.resolved_from_url,
            ),
        },
        "first_seen_at": now,
        "last_seen_at": now,
...
async def _probe_mrf_file_heads(
    *,
    ...
    probe_targets = await _load_file_probe_targets(
