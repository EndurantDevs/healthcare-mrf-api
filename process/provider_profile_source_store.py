...
"""Source-scoped retention and atomic publication of complete state profiles."""
...
class ProfileSourcePolicy:
    """Code-defined identity and source-specific retained-row predicates."""
    ...
    error_prefix: str
    received_profile_sql: str
    invalid_fact_sql: str = "FALSE"
    guarded_public_sql: str = "TRUE"


@dataclass(frozen=True)
class SourceProfileStore:
    """Bind publication and retention rules to one immutable source policy."""
    ...
    def _run_id(self, run_id):
        if not isinstance(run_id, str) or not RUN_ID_PATTERN.fullmatch(run_id):
            raise ValueError(f"{self.policy.error_prefix}_run_id_invalid")
        return run_id
    ...
    async def _read_run(self, run_id):
        ...
        source_row = await db.first(select(table).where(table.c.run_id == self._run_id(run_id)))
    ...
    async def read_publication(self):
        """Return the source pointer only when it refers to a completed full run."""
        table = ProviderProfileSourcePublication.__table__
        source_row = await db.first(select(table).where(table.c.source_key == self.policy.source_key))
        if source_row is None:
            return None
    ...
    def _has_valid_categories(self, categories):
        return categories == list(self.policy.categories)
    ...
    def _manifest(self, run_by_field):
        ...
        if not re.fullmatch(r"[a-f0-9]{64}", str(manifest_by_field["cohort_sha256"])):
            raise ValueError(f"{self.policy.error_prefix}_cohort_hash_invalid")
        for key in ("full_cohort_licenses", "requested_licenses"):
            if type(manifest_by_field[key]) is not int or manifest_by_field[key] <= 0:
                raise ValueError(f"{self.policy.error_prefix}_manifest_count_invalid")
        ...
        for key in ("expected_current_run_id", "resume_from"):
            if manifest_by_field[key] is not None:
                self._run_id(manifest_by_field[key])
        return manifest_by_field

    async def _expected_publication(self, expected_run_id):
        ...
        if current_run_id != expected_run_id:
            raise RuntimeError(f"{self.policy.error_prefix}_predecessor_changed")
        return publication
    ...
    async def update_run(self, run_id, fields: dict):
        ...
        if not isinstance(fields, dict) or not fields or not set(fields) <= {"status", "metrics"}:
            raise ValueError(f"{self.policy.error_prefix}_run_update_invalid")
        if "status" in fields and fields["status"] != "validating":
            raise ValueError(f"{self.policy.error_prefix}_run_transition_invalid")
        if "metrics" in fields and not isinstance(fields["metrics"], dict):
            raise ValueError(f"{self.policy.error_prefix}_metrics_invalid")
        async with db.transaction():
            await self._lock_source()
            ...
            WITH source_counts AS (
                ...
                SELECT f.run_id, count(*) AS retained_facts,
                   ...
                   count(DISTINCT f.npi) FILTER (WHERE r.run_id = f.run_id AND r.source_key = :source_key
                       AND r.match_status = 'deterministic' AND r.matched_npi = f.npi
                       AND r.normalized_payload->>'visibility' = 'public'
                       AND ({self.policy.guarded_public_sql})
    ...
    def _publication_volume(self, metrics, incumbent_metrics):
        if metrics["received_profiles"] * 2 < metrics["requested_licenses"]:
            raise RuntimeError(f"{self.policy.error_prefix}_received_profile_ratio")
        if incumbent_metrics is None:
            if metrics["matched_public_providers"] < 10000:
                raise RuntimeError(f"{self.policy.error_prefix}_first_publication_too_small")
        else:
            for key in ("matched_public_providers", "received_profiles"):
                if metrics[key] * 5 < incumbent_metrics[key] * 4:
                    raise RuntimeError(f"{self.policy.error_prefix}_publication_volume_drop:{key}")
    ...
    async def publish_run(self, run_id, *, expected_current_run_id, metrics):
        ...
        async with db.transaction():
            ...
            if manifest["expected_current_run_id"] != expected_current_run_id:
                raise RuntimeError(f"{self.policy.error_prefix}_frozen_predecessor_mismatch")
            await self._expected_publication(expected_current_run_id)
            final_metrics = self._completion_metrics(candidate_run, metrics, await self.retained_counts(run_id))
    ...
    async def finish_unpublished_run(self, run_id, metrics):
        ...
        async with db.transaction():
            await self._lock_source()
            candidate_run = await self._read_run(run_id)
            if candidate_run["status"] not in ACTIVE_STATUSES or self._manifest(candidate_run)["max_providers"] is None:
                raise RuntimeError(f"{self.policy.error_prefix}_bounded_completion_invalid")
            final_metrics = self._completion_metrics(candidate_run, metrics, await self.retained_counts(run_id))
    ...
    async def read_resume_run(self, run_id, *, max_providers, expected_current_run_id):
    ...
    async def retain_source_history(self, artifact_root):
