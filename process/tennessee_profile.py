...
from process.tennessee_profile_rows import SCHEMA_VERSION, SOURCE_KEY
from process.tennessee_profile_store import IMPORTER, completion, store
...
def _source_manifest(task, snapshot, expected):
    ...
    return {
        ...
        "report_professions": sorted(binding.PROFESSIONS),
        ...
        "source": {
            ...
            "source_url": acquisition.BASE_URL,
...
def _read_reports(directory, acquired, run_id):
    binding._require(
        acquired.get("complete") is True
        and set(acquired.get("reports", {})) == binding.PROFESSIONS
        and len(acquired.get("responses", [])) == 8
        and all(
            response.get("complete") is True and response.get("status") == 200 for response in acquired["responses"]
        ),
    ...
    for label, _, profession, _ in acquisition.REPORTS:
        ...
        _reject_symlinks(path)
        binding._require(
            receipt["filepath"] == str(path)
            and receipt["source_url"] == acquisition.REPORT_URL
            and 0 < path.stat().st_size <= acquisition.MAX_REPORT_BYTES,
            "report_file_invalid",
        )
        content = path.read_bytes()
        binding._require(
            len(content) == receipt["content_bytes"] <= acquisition.MAX_REPORT_BYTES
        ...
        reports[profession] = {
            "content": content,
            "evidence": {**evidence_by_field, "run_id": run_id, "artifact_id": artifact_id},
        }
...
def _artifact(run_by_field, directory, descriptors, acquired, bound):
    profession_by_record = {
        source_record["record_id"]: source_record["profession_code"] for source_record in bound["source_records"]
    }
    metrics_by_field = {
        "acquisition_complete": True,
        "transport_failures": 0,
        "http_responses": len(acquired["responses"]),
        "report_responses": len(descriptors),
        "response_bytes": sum(response["content_bytes"] for response in acquired["responses"]),
        "reports_sha256": bound["reports_sha256"],
    ...
    metrics_by_field["facts_by_profession"].update(
        Counter(profession_by_record[fact["source_record_id"]] for fact in bound["facts"])
    )
    bundle_by_field = {
        ...
        "snapshot": {
            "content_sha256": bound["snapshot_sha256"],
...
async def _persist_profiles(ctx, task, artifact, bound):
    ...
    for model, rows, identity in (
        (ProviderProfileSourceRecord, bound["source_records"], "record_id"),
...
async def _run_claimed(ctx, task, run_by_field, snapshot, directory):
    ...
    bound = await _bind_reports(
        reports,
        binding.reports_content_sha256(reports),
        directory,
        run_by_field["source_manifest"],
...
async def import_profiles(ctx, task):
    ...
    run_by_field = {
        ...
        "schema_version": SCHEMA_VERSION,
        "status": "running",
        "source_manifest": _source_manifest(task, snapshot, expected),
        "metrics": {},
        "error": None,
        "started_at": _now(),
        "finished_at": None,
