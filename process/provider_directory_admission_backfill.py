...
"""Transactional backfill for legacy Provider Directory admission seals."""
...
def _qualified_dataset_table() -> str:
    ...
    return f'"{schema}"."provider_directory_endpoint_dataset"'


async def _locked_dataset_row(
    connection: Any,
    dataset_ref: str,
    dataset_id: str,
) -> Any:
    return await connection.fetchrow(
        ...
        SELECT dataset_id, endpoint_id,
               ...
               CASE WHEN completion_proof_required_version = 3
...
def _existing_seal_result(
    ...
    seal_values = tuple(dataset_row[field] for field in _SEAL_FIELDS)
...
def _validated_row_metadata_size(dataset_row: Any) -> int:
...
async def _copy_metadata(
    ...
    async def spool_copy(copy_chunk: bytes) -> None:
        """Write one bounded database COPY chunk to the private spool."""
...
async def _validated_row_seal(
    ...
    with tempfile.TemporaryDirectory(
        ...
        copy_path = await _copy_metadata(
            connection,
            dataset_ref,
            dataset_row,
            temporary_path,
...
async def _backfill_locked_row(
    connection: Any,
    dataset_ref: str,
    dataset_id: str,
) -> dict[str, Any]:
    dataset_row = await _locked_dataset_row(connection, dataset_ref, dataset_id)
    if dataset_row is None:
        raise AdmissionSealError("provider_directory_admission_dataset_missing")
    existing_result = _existing_seal_result(dataset_id, dataset_row)
    if existing_result is not None:
        return existing_result
    raw_metadata_bytes = _validated_row_metadata_size(dataset_row)
    seal = await _validated_row_seal(connection, dataset_ref, dataset_row)
    await _store_seal(connection, dataset_ref, dataset_row, seal)
    return {
        "dataset_id": dataset_id,
        "status": "sealed",
    ...
    *,
    ...
    async with database.acquire_driver() as connection:
        async with connection.transaction(isolation="repeatable_read"):
            return await _backfill_locked_row(
                connection,
                dataset_ref,
                dataset_id,
            )
