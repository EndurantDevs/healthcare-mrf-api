...
def _admission_from_row(database_row: Any) -> UHCFlexPractitionerTwinAdmission:
    database_fields = row_fields(database_row)
    if not database_fields:
...
def _require_exact_admission(
    stored: UHCFlexPractitionerTwinAdmission,
    expected: UHCFlexPractitionerTwinAdmission,
) -> None:
    if _identity_values(stored, _ADMISSION_IDENTITY_COLUMNS) != _identity_values(
        expected,
...
async def require_uhc_flex_practitioner_admission(
    ...
    *,
    ...
    async with database.transaction():
        ...
        if (
            admission.semantic_projection_as_of != projection_date
            or admission.operation_key != expected_operation_key
        ):
            raise UHCFlexPractitionerTwinStoreError("identity")
