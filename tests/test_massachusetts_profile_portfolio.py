...
def test_api_preserves_education_and_original_board_support_in_each_page():
    ...
    for category in ("certifications", "specialties"):
        ...
        for offset in range(count):
            ...
            records = evidence["sources"][rows.SOURCE_KEY]["records"]
        ...
        assert seen_fact_ids == {item["fact_id"] for item in _parse()[1] if item["category"] == category}
...
def test_equal_boards_keep_original_assertions():
    profile_by_field = {**PORTFOLIO, "boardCertifications": {"abms": [BOARD, BOARD], "aoa": [BOARD]}}
    projection = state_api.merge_state_profile_projection(NPI, None, _projection(profile_by_field))
    original = copy.deepcopy(projection)
    original_boards = original["profile"]["categories"]["certifications"]["items"]
    assert len(original_boards) == 3
    full = _compose(projection, requested_categories=["certifications"])
    board_facts = full["categories"]["certifications"]["items"]
    assert len(board_facts) == len({board_fact["item_id"] for board_fact in board_facts}) == 2
    assert {board_fact["value"]["board_family"] for board_fact in board_facts} == {"abms", "aoa"}
    evidence = profile_api.compose_provider_profile_evidence(
        state_projection=projection, fhir_evidence=None, provider_profile=full, page_category="certifications",
    )
    original_evidence_records = [evidence_record for evidence_record in original["additional_state_evidence"][rows.SOURCE_KEY]["records"]
                         if evidence_record["source_path"].startswith("boardCertifications.")]
    assert evidence["sources"][rows.SOURCE_KEY]["records"] == original_evidence_records
    for offset, board_fact in enumerate(board_facts):
        family = board_fact["value"]["board_family"]
        assert board_fact["assertion_count"] == len(board_fact["assertions"]) == len(board_fact["source_record_ids"]) == (2 if family == "abms" else 1)
        assert board_fact["assertions"] == [assertion for original_board in original_boards
                                            if original_board["value"] == board_fact["value"] for assertion in original_board["assertions"]]
        page = _compose(projection, requested_categories=["certifications"], page_category="certifications", page_limit=1, page_offset=offset)
        assert page["categories"]["certifications"]["items"] == [board_fact]
        assert page["categories"]["certifications"]["total"] == 2
        evidence = profile_api.compose_provider_profile_evidence(
            state_projection=projection, fhir_evidence=None, provider_profile=page, page_category="certifications",
        )
        evidence_records = evidence["sources"][rows.SOURCE_KEY]["records"]
        assert evidence_records == [evidence_record for evidence_record in original_evidence_records
                                    if evidence_record["source_record_id"] in board_fact["source_record_ids"]]
        assert {evidence_record["source_record_id"] for evidence_record in evidence_records} == set(board_fact["source_record_ids"])
        assert {evidence_record["source_path"] for evidence_record in evidence_records} == (
            {"boardCertifications.abms[0]", "boardCertifications.abms[1]"} if family == "abms" else {"boardCertifications.aoa[0]"}
        )
        assert all(evidence_record["raw_fields"] == BOARD for evidence_record in evidence_records)
    assert _compose(projection, page_category="certifications", page_limit=1, page_offset=2)["categories"]["certifications"]["items"] == []
