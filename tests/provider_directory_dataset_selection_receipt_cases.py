...
"""PostgreSQL contracts for bounded selection and receipt publication."""
...
async def _store_sealed_canonical_shared_dataset(
    database,
    schema: str,
    metadata_by_field: dict[str, object],
    receipt,
) -> None:
    """Store one oversized canonical proof behind its bounded receipt."""
...
async def _assert_sealed_bypasses_legacy_read(database, schema: str) -> None:
    """Prove a valid seal bypasses the legacy raw-metadata predicate."""
...
async def test_sealed_canonical_proof_uses_bounded_receipt_over_legacy_cap(
    ...
    async with _dataset_database(monkeypatch) as (database, schema):
        ...
        await _store_sealed_canonical_shared_dataset(
            database,
            schema,
            metadata_by_field,
            receipt,
        )
        await _assert_sealed_bypasses_legacy_read(database, schema)
