...
def _rows(*source_rows, generation=GENERATION):
    content = report_bytes(source_rows or [source_row(
        EducationProvider="Example Medical School", GraduationDate="06/01/2001",
    )])
    _, facts = parse_report(content, evidence={
        **EVIDENCE, "run_id": generation, "content_sha256": hashlib.sha256(content).hexdigest(),
    })
    return [{
        **fact, "npi": NPI, "publication_source_key": SOURCE_KEY, "generation_id": generation,
        "source_published_at": datetime(2026, 9, 10), "run_status": "completed",
        "run_schema_version": SCHEMA_VERSION, "run_jurisdiction": "TN",
        "source_manifest": {"categories": ["education", "training", "specialties"], "source": {
            "source_key": SOURCE_KEY, "source_kind": "state_regulator", "agency": "Tennessee Department of Health",
            "jurisdiction": "TN", "coverage_scope": "full_md_do_reports", "registry_generation": "a" * 64,
        }},
    } for fact in facts]
...
def test_reported_portfolio_keeps_literal_values_uncertainty_and_per_fact_evidence():
    ...
    assert [record["raw_fields"] for record in projection["evidence"]["records"]] == [row["source_json"]["raw_fields"] for row in rows]
...
def test_tennessee_cms_and_florida_corroborate_school_and_preserve_distinct_education():
    ...
    rows = _rows(source_row(EducationProvider="EXAMPLE Medical School", GraduationDate="06/01/2001"),
                 source_row(EducationProvider="Second Medical School", GraduationDate="06/01/2006"))
    projection = _envelope(*rows, incumbent=incumbent)
    profile = _compose(projection)
    items = profile["categories"]["education"]["items"]
    assert len(items) == 2
    corroborated_items = [item for item in items if item["assertion_count"] == 3]
    assert len(corroborated_items) == 1
    corroborated = corroborated_items[0]
    assert len(corroborated["assertions"]) == 3
    assert {source["source_key"] for source in profile["sources"]} == {SOURCE_KEY, "cms-doctors", "florida-mqa"}
    assert {assertion["assertion_type"] for assertion in corroborated["assertions"]} == {
        "source_reported", "cms_reported", "practitioner_reported",
    }
    assert corroborated["corroborated_fields"] == ["institution", "graduation_year"]
    tennessee_assertions = [assertion for assertion in corroborated["assertions"] if assertion.get("source_ids") == [SOURCE_KEY]]
    assert len(tennessee_assertions) == 1 and tennessee_assertions[0]["value"] == rows[0]["value_json"]
    separate_items = [item for item in items if item["assertion_count"] == 1]
...
def test_distinct_degrees_dates_and_training_remain_distinct_with_exact_specialty_corroboration():
    rows = _rows(source_row(), source_row(DegreeEarned="PHD"), source_row(GraduationDate="06/16/1998"),
...
def test_each_page_has_exact_supporting_fact_evidence(category):
    rows = _rows(source_row(EducationProvider="Example Medical School", GraduationDate="06/01/2001"),
                 source_row(EducationProvider="Second Medical School", GraduationDate="06/01/2006",
                            OtherTrainingProvider="Second Hospital", ModifierDescription="Pediatrics"))
    incumbent = cms_api.merge_cms_education_projection(NPI, _legacy_envelope(), _cms_projection())
    projection = _envelope(*rows, incumbent=incumbent)
    seen_fact_ids = set()
    for offset in range(2):
        profile = _compose(projection, requested_categories=[category], page_category=category, page_limit=1, page_offset=offset)
        item, = profile["categories"][category]["items"]
        evidence = profile_api.compose_provider_profile_evidence(
            state_projection=projection, fhir_evidence=None, provider_profile=profile, page_category=category,
        )
        assert {record["source_record_id"] for source in evidence["sources"].values() for record in source["records"]} == set(item["source_record_ids"])
...
def test_tennessee_rotation_changes_generation_without_changing_logical_item_identity():
    ...
    for category in ("education", "training", "specialties"):
        assert before["categories"][category]["items"][0]["item_id"] == after["categories"][category]["items"][0]["item_id"]


@pytest.mark.parametrize(("field", "value", "reason"), [
    ("run_status", "running", "publication_invalid"), ("run_status", "failed", "publication_invalid"),
    ("run_schema_version", "ma-borim-profile/v1", "source_mismatch"), ("run_jurisdiction", "MA", "source_mismatch"),
    ...
    ("category", "services", "categories_invalid"), ("fact_type", "postgraduate_training", "categories_invalid"),
])
...
@pytest.mark.parametrize(("field", "value"), [
    ("source_key", "massachusetts-borim"), ("source_kind", "cms_doctors"),
    ("jurisdiction", "MA"), ("agency", "Other board"),
])
...
@pytest.mark.parametrize(("field", "value"), [
    ("source_key", "massachusetts-borim"), ("source_record_id", "other-profile"),
...
@pytest.mark.parametrize("categories", [
    [], ["education"], ["education", "training"], ["education", "training", "certifications"],
    ["education", "training", "specialties", "services"], ["education", "training", "specialties", "specialties"],
])
...
@pytest.mark.parametrize("changes", [
    {"fact_id": None}, {"sensitive": True}, {"public_default": False}, {"availability": "restricted"},
])
