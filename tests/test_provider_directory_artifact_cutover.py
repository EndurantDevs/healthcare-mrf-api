...
async def test_real_postgres_artifact_bundle_accepts_timeout_after_commit(monkeypatch):
    ...
    async with _dataset_database(monkeypatch) as (database, schema):
        ...
        async def commit_then_lose_acknowledgement(prepared_stages):
            await promote_transaction(prepared_stages)
            raise TimeoutError
        ...
        for stage_name in ("artifact_stage_a", "artifact_stage_b"):
            assert await database.scalar(
                "SELECT to_regclass(:relation_name);",
                relation_name=f"{schema}.{stage_name}",
            ) is None
