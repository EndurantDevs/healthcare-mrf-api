...
def _form(board, submitted=False):
    ...
    checkboxes = "".join(
        f'<input type="checkbox" name="{field}" value="true"'
    ...
    hidden += "".join(
        f'<input type="hidden" name="{field}" value="false">'
        for field in ("PersonFlag", "EduFlag", "PracticeFlag", "SAQFlag")
    )
    download = (
        '<input type="hidden" name="hasFile" value="True"><button id="hidden-button" type="submit" hidden/>'
        if submitted
        else ""
    )
    return (
        f'<form action="/LicensureReports" method="post">{selects}{checkboxes}{hidden}'
        '<button id="submit-button" type="submit">Submit</button></form>'
        f'<form action="/LicensureReports/Home/CreateFile" method="get">{download}</form>'
    ).encode()
    ...
    for board, code, label in [("16", 1606, "Medical Doctor"), ("19", 1907, "Osteopathic Physician")]:
        responses.extend(
            [
                ...
                SourceResponse(
                    json.dumps([{"professionCode": code, "professionName": label}]).encode(), "application/json"
                ),
...
async def test_two_report_unit_retains_exact_bytes_and_token_free_receipts(tmp_path, source_sessions):
    ...
    for session, board, code in zip(sessions, ("16", "19"), ("1606", "1907")):
        ...
        assert [(method, url) for method, url, _ in session.requests] == [
            ...
            ("POST", acquisition.PROFESSIONS_URL),
            ("POST", acquisition.BASE_URL),
            ("GET", acquisition.REPORT_URL),
        ]
    ...
    assert "synthetic-session-token" not in json.dumps(manifest) and "synthetic-private-cookie" not in json.dumps(
        manifest
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            2,
            b'<select name="Status.Status"></select>',
            b'<select name="Status.Status"><option selected>Active</option></select>',
        ),
    ],
)
...
async def test_cancellation_never_marks_unit_complete(tmp_path, source_sessions, boundary):
    ...
    async def progress(completed, total):
        ...
        if (
            (boundary == "before")
            or (boundary == "chunk" and responses[0].reads == 1)
            or (boundary == "between" and completed == 4)
            or (boundary == "final" and completed == 8)
        ):
    ...
    assert (
        sum(len(session.requests) for session in sessions)
        == {"before": 0, "chunk": 1, "between": 4, "final": 8}[boundary]
    )
    if boundary == "chunk":
...
async def test_unknown_paths_are_never_reused_or_removed(tmp_path, source_sessions, kind):
    ...
    assert (
        marker.read_bytes() == b"incumbent"
        if kind == "existing"
        else (marker if kind == "symlink" else directory).is_symlink()
    )
...
@pytest.mark.parametrize(
    "stage,header,header_value",
    [
        (0, "Content-Type", "application/xhtml+xml"),
        (1, "Content-Type", "text/html"),
        (0, "Content-Length", str(2 * 1024 * 1024 + 1)),
        (1, "Content-Length", "-1"),
    ],
)
