...
from process.ptg_parts import (
    ptg2_partitioned_candidate_audit_request_contract as request_contract,
)
...
def test_plan_is_deterministic_max_25_and_exact_once():
    plan = _plan()
    repeated = _plan()

    assert plan == repeated
    assert [request.item_count for request in plan.requests] == [25] * 8 + [3]
    assert all(
        request.item_count
    ...
    assert len({request.request_digest for request in plan.requests}) == 9
    assert all(
...
def test_smaller_partitions_preserve_the_sealed_plan_identity(monkeypatch):
    bounded_plan = _plan()
    ...
    assert bounded_plan.plan_digest == legacy_plan.plan_digest
    assert (
        bounded_plan.source_challenge_count
        == legacy_plan.source_challenge_count
    )
    assert (
        bounded_plan.source_occurrence_count
        == legacy_plan.source_occurrence_count
    )
    assert (
        bounded_plan.persisted_occurrence_count
        == legacy_plan.persisted_occurrence_count
    )
    assert [request.item_count for request in legacy_plan.requests] == [
        50,
    ] * 4 + [3]
    assert all(
        contract.parse_partitioned_candidate_audit_request(request.payload)
        == request
        for request in legacy_plan.requests
    )
...
def test_plan_preserves_code_groups_when_they_fit():
    ...
    assert [request.item_count for request in plan.requests] == [25] * 3 + [
        6,
        25,
        5,
    ]
...
def test_results_parse_and_aggregate_complete_plan():
    ...
    assert aggregate.block_io["physical_block_reads"] == 9
