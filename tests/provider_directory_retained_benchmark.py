...
from tests.provider_directory_retained_benchmark_evidence import (
    MAXIMUM_PUBLICATION_SECONDS,
    MINIMUM_RESOURCES_PER_SECOND,
    _assert_database_quiescent,
    _assert_publication_state,
    _benchmark_event_map,
    _live_relation_lifecycle_proof,
    _live_relation_snapshot,
    _stage_lifecycle_proof,
    _stage_relation_snapshot,
)
...
@contextmanager
def _benchmark_patch_context(modules, importer, observations):
    patch_specs = (
        ...
        (
            importer,
            "_publish_or_replay_uhc_candidate",
            observations.publish_candidate,
        ),
    ...
    with ExitStack() as stack:
        for patch_target, attribute_name, replacement in patch_specs:
            stack.enter_context(patch.object(patch_target, attribute_name, replacement))
        yield
...
async def _run_benchmark_imports(modules, source_row, import_task, run_id):
    ...
    with _benchmark_patch_context(modules, importer, observations):
        started_at = time.perf_counter()
...
async def _execute_benchmark(modules, catalog_hash, encoder_digest, database_name):
    ...
    publication_proof_by_name, cleanup_proof_by_name = await _assert_publication_state(
        modules.importer,
        observations,
        candidate,
        publication_map,
    ...
    return _benchmark_event_map(
        ...
        first_stage,
        {"fresh": pipeline_seconds, "replay": replay_pipeline_seconds},
    )
