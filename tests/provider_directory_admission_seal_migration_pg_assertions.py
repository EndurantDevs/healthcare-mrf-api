...
"""PostgreSQL assertions for the admission-seal migration."""
...
async def _assert_invalid_write_paths(connection, schema: str) -> None:
    await _assert_partial_write_paths(connection, schema)
    await _assert_invalid_resource_arrays(connection, schema)


async def _assert_partial_write_paths(connection, schema: str) -> None:
...
async def _assert_invalid_complete_receipts(connection, schema: str) -> None:
    ...
    for index, (
        ...
        resource_types,
        digest_override,
    ) in enumerate(_INVALID_RECEIPTS):
...
async def _assert_sealed_mutations(connection, schema: str) -> None:
    ...
    await _assert_raw_and_summary_mutations_rejected(connection, schema, table)
    await _assert_additive_summary_update_scoped(connection, schema, table)
    await _assert_receipt_and_truncate_mutations_rejected(connection, schema, table)


async def _assert_raw_and_summary_mutations_rejected(
    connection,
    schema: str,
    table: str,
) -> None:
    await _expect_error(
        ...
        f"""UPDATE {table}
           SET publication_metadata_json = pg_catalog.jsonb_set(
               publication_metadata_json::jsonb, '{{reviewed}}',
               'true'::jsonb, true)::json
         WHERE dataset_id = 'dataset_sealed'""",
    )
    await _expect_error(
        connection,
...
async def _assert_additive_summary_update_scoped(
    connection,
    schema: str,
    table: str,
) -> None:
    await connection.execute(
        f"""UPDATE {table}
           SET publication_metadata_summary_json = jsonb_set(
                   ...
                   '{{"complete":true}}'::jsonb, true),
               publication_metadata_sha256 =
                   "{schema}".provider_directory_endpoint_dataset_admission_metadata_sha256(
                       jsonb_set(publication_metadata_summary_json,
                           '{{outcome_resource_counts_v1}}',
                           '{{"complete":true}}'::jsonb, true),
                       content_proof_admission_version, content_proof_admission_kind,
                       content_proof_admission_sha256, content_proof_resource_types)
...
async def _assert_receipt_and_truncate_mutations_rejected(
    connection,
    schema: str,
    table: str,
) -> None:
           ...
           SET content_proof_admission_sha256 = repeat('b', 64),
               publication_metadata_sha256 =
                   "{schema}".provider_directory_endpoint_dataset_admission_metadata_sha256(
                       publication_metadata_summary_json,
                       content_proof_admission_version, content_proof_admission_kind,
                       repeat('b', 64), content_proof_resource_types)
         WHERE dataset_id = 'dataset_sealed'""",
    )
    await _expect_error(
        connection,
