...
def _run():
    return {
        "run_id": "1" * 64, "source_key": tennessee.SOURCE_KEY, "jurisdiction": "TN",
        "schema_version": tennessee.SCHEMA_VERSION, "status": "running", "started_at": shared_store._now(),
        "source_manifest": {
            "control_run_id": "run_tennessee_synthetic", "expected_current_run_id": None,
            "max_providers": None, "resume_from": None, "categories": list(tennessee.CATEGORIES),
            "report_professions": list(tennessee.PROFESSIONS), "snapshot_sha256": "a" * 64, "snapshot_row_count": 12,
            "source": {"source_key": tennessee.SOURCE_KEY, "source_kind": "state_regulator", "jurisdiction": "TN",
                       "agency": "Tennessee Department of Health", "source_url": tennessee.SOURCE_URL,
                       "coverage_scope": tennessee.COVERAGE_SCOPE, "registry_generation": "a" * 64},
        },
    }
...
def _report_inputs(run_id="1" * 64):
    ...
    return {profession: {"content": profession.encode(), "evidence": {
        "run_id": run_id, "artifact_id": artifact_id,
        "content_sha256": hashlib.sha256(profession.encode()).hexdigest(),
        "source_url": tennessee.SOURCE_URL + "/synthetic.csv", "downloaded_at": "2026-09-10T00:00:00+00:00",
    }} for profession in tennessee.PROFESSIONS}


def _metrics():
    return {
        "acquisition_complete": True, "transport_failures": 0, "http_responses": 8, "report_responses": 2,
        "reports_sha256": binding.reports_content_sha256(_report_inputs()),
        "snapshot_sha256": "a" * 64, "response_bytes": 168129657,
...
def _artifact(run_by_field=None, metrics_by_field=None):
    ...
    bundle_by_field = {
        "schema_version": tennessee.SCHEMA_VERSION, "run_id": run_by_field["run_id"],
        "source_manifest": deepcopy(run_by_field["source_manifest"]), "reports_sha256": metrics_by_field["reports_sha256"],
        ...
        "reports": {profession: {
            **{field: report["evidence"][field] for field in ("content_sha256", "source_url", "downloaded_at")},
            "content_bytes": len(report["content"]), "file_name": profession + ".csv",
    ...
    return {
        ...
        "run_id": run_by_field["run_id"], "source_key": tennessee.SOURCE_KEY, "category": "profile",
        "file_name": "manifest.json", "source_url": tennessee.SOURCE_URL, "metadata_json": bundle_by_field,
        "content_sha256": hashlib.sha256(canonical).hexdigest(), "content_bytes": len(canonical),
    }
...
def _counts():
    ...
    return {
        "retained_source_records": 77048, "received_profiles": 77048, "retained_facts": 280952,
        "matched_public_providers": 9894, "portfolio_only_public_providers": 84,
        "invalid_source_records": 0, "invalid_facts": 0, "foreign_artifacts": 0,
        "invalid_bundle_records": 0, "invalid_report_facts": 0, "retained_artifacts": 1,
        "md_source_records": 70018, "do_source_records": 7030, "md_facts": 258427, "do_facts": 22525,
...
def test_source_policy_uses_shared_lifecycle_without_mutable_configuration():
    ...
    assert progress == {"unit": "record", "done": 77048, "total": 77048, "pct": 100,
                        "phase": "tennessee-tdh-profile published", "message": "succeeded"}


@pytest.mark.parametrize("changes", [
    {"max_providers": 1}, {"max_providers": False}, {"resume_from": "2" * 64},
    {"categories": ["education"]}, {"report_professions": ["1606"]}, {"report_professions": ["1907", "1606"]},
    {"snapshot_sha256": "invalid"}, {"snapshot_row_count": -1}, {"snapshot_row_count": True},
    {"expected_current_run_id": "../foreign"}, {"control_run_id": " "},
    {"requested_licenses": 2}, {"full_cohort_licenses": 77048},
])
...
@pytest.mark.parametrize(("field", "value"), [
    ("source_key", "florida-mqa"), ("source_kind", "cms_doctors"), ("jurisdiction", "MA"),
    ("agency", "Other board"), ("source_url", "https://example.test"), ("coverage_scope", "physicians_only"),
...
@pytest.mark.parametrize(("field", "value"), [
    ("acquisition_complete", False), ("acquisition_complete", 1), ("transport_failures", 1),
    ("transport_failures", False), ("http_responses", 7), ("http_responses", True), ("report_responses", 1),
    ("response_bytes", 0), ("response_bytes", True), ("reports_sha256", "f" * 64), ("snapshot_sha256", "e" * 64),
    ("source_records_by_profession", {"1606": 77048}), ("facts_by_profession", {"1606": 258427, "1907": True}),
])
...
@pytest.mark.parametrize("field", [
    "invalid_source_records", "invalid_facts", "foreign_artifacts", "invalid_bundle_artifacts",
    "invalid_bundle_records", "invalid_report_facts", "retained_source_records", "received_profiles", "retained_facts",
])
...
@pytest.mark.parametrize(("field", "minimum"), [
    ("md_source_records", 60000), ("do_source_records", 6000), ("matched_public_providers", 8000),
])
...
@pytest.mark.parametrize("field", ["md_source_records", "do_source_records", "matched_public_providers", "received_profiles"])
...
@pytest.mark.parametrize("change", ["missing", "duplicate", "wrong_id", "changed_metadata", "missing_do", "same_file", "snapshot"])
...
@pytest.mark.parametrize(("field", "changed"), [
    ("content_sha256", "e" * 64), ("source_url", "https://example.test/changed.csv"),
...
async def _seed_native(database):
    candidate = _run()
    metrics_by_field = {**_metrics(), "source_records_by_profession": {"1606": 3, "1907": 1},
    ...
    for number, profession, visibility in ((1, "1606", "public"), (2, "1606", "held_identity"),
                                           (3, "1606", "public"), (4, "1907", "public")):
        record_id = str(number) * 64
        npi = 1000000004 if number == 1 else 1000000012 if number == 4 else None
        source_by_field = {
            "record_id": record_id, "run_id": candidate["run_id"], "artifact_id": artifact["artifact_id"],
            "source_key": tennessee.SOURCE_KEY, "source_record_key": f"tennessee-tdh:{profession}:{number}",
            "profession_code": profession, "license_number": None if number == 2 else str(number),
            ...
            "matched_npi": npi, "match_status": "deterministic" if npi else "unmatched",
            "match_evidence": {"registry_binding": {"snapshot_sha256": "a" * 64,
                "reports_sha256": metrics_by_field["reports_sha256"], "status": "deterministic" if npi else "unmatched", "npi": npi}},
        }
        ...
        fact_by_field = {
            "fact_id": record_id, "run_id": candidate["run_id"], "npi": npi, "source_record_id": record_id,
            "logical_fact_key": record_id, "category": "education", "fact_type": "education_history",
            "display": "Synthetic School", "value_json": {"institution": "Synthetic School"},
            "availability": "available", "assertion_type": "source_reported", "verification_status": "not_independently_verified",
            "source_json": {"source_key": tennessee.SOURCE_KEY, "schema_version": tennessee.SCHEMA_VERSION,
                "source_record_id": record_id, "run_id": candidate["run_id"], "artifact_id": artifact["artifact_id"],
                "agency": "Tennessee Department of Health", "jurisdiction": "TN",
...
async def test_native_retained_counts_include_unlicensed_held_and_blank_profiles(monkeypatch):
    async with _database(monkeypatch) as database:
        ...
        with pytest.raises(RuntimeError, match="first_publication_too_small"):
            await tennessee.store.publish_run(candidate["run_id"], expected_current_run_id=None, metrics=metrics_by_field)
...
async def test_native_real_binding_bundle_and_store_agree(monkeypatch, tmp_path):
    ...
    for profession, report in report_by_profession.items():
        report["evidence"].update(run_id=candidate["run_id"], source_url=worker.acquisition.REPORT_URL,
        ...
        descriptors_by_profession[profession] = {
            **{field: report["evidence"][field] for field in ("content_sha256", "source_url", "downloaded_at")},
            "file_name": file_name, "content_bytes": len(report["content"]),
        }
    bound = binding.bind_reports(report_by_profession,
        reports_sha256=binding.reports_content_sha256(report_by_profession), snapshot_path=snapshot_path,
        snapshot_sha256=candidate["source_manifest"]["snapshot_sha256"])
    artifact, metrics_by_field = worker._artifact(candidate, tmp_path, descriptors_by_profession,
        {"responses": [{"content_bytes": 100} for _ in range(8)]}, bound)
    async with _database(monkeypatch) as database:
        await tennessee.store.claim_run(candidate)
        await database.insert(shared_store.ProviderProfileArtifact.__table__).values(artifact).status()
        await database.insert(shared_store.ProviderProfileSourceRecord.__table__).values(bound["source_records"]).status()
        ...
        assert all(counts[name] == 0 for name in (
            "invalid_source_records", "invalid_facts", "foreign_artifacts", "invalid_bundle_artifacts",
            "invalid_bundle_records", "invalid_report_facts"))
        ...
        assert await tennessee.store.read_publication() is None
        assert (await tennessee.store._read_run(candidate["run_id"]))["status"] == "running"
        assert await database.scalar(f"SELECT count(*) FROM {tennessee.store._table(shared_store.ProviderProfileFact)} "
                                     "WHERE published_at IS NOT NULL") == 0
...
async def test_native_corrupt_report_pair_or_fact_blocks_completion_and_retention(monkeypatch, change):
    async with _database(monkeypatch) as database:
        ...
        match change:
            case "report_fact":
                await database.status(f"UPDATE {tennessee.store._table(shared_store.ProviderProfileFact)} "
            ...
            case "held_fact":
                await database.update(source_table).where(source_table.c.record_id == "1" * 64).values(
                    normalized_payload={"schema_version": tennessee.SCHEMA_VERSION, "visibility": "held_identity"}).status()
            case "snapshot_binding":
                await database.update(source_table).values(match_evidence={"registry_binding": {"snapshot_sha256": "f" * 64}}).status()
            case "profession":
        ...
        assert await database.scalar(f"SELECT count(*) FROM {tennessee.store._table(shared_store.ProviderProfileFact)}") == 2


@pytest.mark.parametrize(("field", "changed"), [
    ("status", "identity_conflict"), ("status", None), ("status", 1),
    ("npi", None), ("npi", "1000000004"), ("npi", 1000000012),
])
async def test_native_top_level_identity_cannot_contradict_retained_binding(monkeypatch, field, changed):
    async with _database(monkeypatch) as database:
        candidate, metrics_by_field = await _seed_native(database)
        source_table = shared_store.ProviderProfileSourceRecord.__table__
        decision_by_field = {"snapshot_sha256": "a" * 64, "reports_sha256": metrics_by_field["reports_sha256"],
                             "status": "deterministic", "npi": 1000000004, field: changed}
        await database.update(source_table).where(source_table.c.record_id == "1" * 64).values(
            match_evidence={"registry_binding": decision_by_field}).status()
        counts = await tennessee.store.retained_counts(candidate["run_id"])
...
async def test_native_missing_binding_identity_is_not_equivalent_to_json_null(monkeypatch, field):
    async with _database(monkeypatch) as database:
        ...
        source_table = shared_store.ProviderProfileSourceRecord.__table__
        decision_by_field = {"snapshot_sha256": "a" * 64, "reports_sha256": metrics_by_field["reports_sha256"],
                             "status": "unmatched", "npi": None}
        del decision_by_field[field]
        await database.update(source_table).where(source_table.c.record_id == "2" * 64).values(
            match_evidence={"registry_binding": decision_by_field}).status()
        counts = await tennessee.store.retained_counts(candidate["run_id"])
        assert counts["invalid_bundle_records"] == 1
        with pytest.raises(RuntimeError, match="retained_integrity_invalid"):
