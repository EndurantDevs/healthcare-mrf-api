...
@pytest.mark.parametrize(
    "params", [[], {"reprocessing_from": "a" * 64}, {"test": True}, {**PARAMS, "resume_from": "b" * 64}]
)
...
async def test_existing_request_must_match_mode_parent_and_bound(monkeypatch, lookup, same_request):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=existing_by_field if lookup == "idempotency" else None),
    )
    monkeypatch.setattr(
        control_imports, "find_earliest_active_run_by_importer", AsyncMock(return_value=existing_by_field)
    )
...
async def test_retained_reprocessing_idempotency_includes_terminal_runs_only_for_that_mode(monkeypatch):
    ...
    assert await control_imports._idempotent_import_run(IMPORTER, "key", params={"resume_from": "a" * 64}) == {
        "run_id": "active"
    }
...
async def test_terminal_retained_key_cannot_be_reused_for_a_network_request(monkeypatch):
    ...
    with pytest.raises(ValueError, match="existing_request_mismatch"):
        await control_imports.create_import_run(
            {"importer": IMPORTER, "params": {}, "idempotency_key": "synthetic-key"}
        )
...
async def test_idempotency_race_rechecks_the_exact_retained_request(monkeypatch):
    ...
    monkeypatch.setattr(
        control_imports, "find_importer_run_by_idempotency_key", AsyncMock(side_effect=[None, existing_by_field])
    )
    monkeypatch.setattr(control_imports, "find_earliest_active_run_by_importer", AsyncMock(return_value=None))
    monkeypatch.setattr(
        control_imports,
        "_admit_import_row",
        AsyncMock(side_effect=IntegrityError("insert", {}, Exception("synthetic collision"))),
    )
    with pytest.raises(ValueError, match="existing_request_mismatch"):
        await control_imports.create_import_run(
            {"importer": IMPORTER, "params": PARAMS, "idempotency_key": "synthetic-key"}
        )
...
async def test_locked_admission_preserves_legacy_terminal_keys_and_checks_active_scope(monkeypatch, state):
    ...
    connection = SimpleNamespace(
        ...
        status=AsyncMock(),
        all=AsyncMock(
            return_value=[SimpleNamespace(_mapping=existing_by_field)]
            if state in {"legacy_terminal", "retained_terminal"}
            else []
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=[existing_by_field] if state in {"same_active", "different_active"} else []),
    )
    request_by_field = {
        "run_id": "synthetic-new",
        "importer": IMPORTER,
        "status": "queued",
    ...
    if state == "different_active":
        ...
        admitted_run = await control_imports._admit_massachusetts_import_run(request_by_field)
        assert (admitted_run is not None) == (state in {"same_active", "retained_terminal"})
...
def test_registry_cli_and_worker_payload_expose_the_same_reprocessing_parameter():
    ...
    payload = control_imports._adapter_payload(
        control_imports._SINGLE_JOB_ADAPTERS[IMPORTER], {"run_id": "synthetic-control", "importer": IMPORTER}, PARAMS
    )
