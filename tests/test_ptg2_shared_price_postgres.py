...
WITH dictionary_source AS (
    SELECT 'negotiated_type'::varchar(64) AS attr_kind,
           negotiated_type::text AS text_value, NULL::text[] AS text_array
      FROM {schema}.price_atoms GROUP BY negotiated_type
    UNION ALL
    SELECT 'expiration_date'::varchar(64), expiration_date::text, NULL::text[]
      FROM {schema}.price_atoms GROUP BY expiration_date
    UNION ALL
    SELECT 'service_code'::varchar(64), NULL::text, service_code::text[]
      FROM {schema}.price_atoms GROUP BY service_code
    UNION ALL
    SELECT 'billing_class'::varchar(64), billing_class::text, NULL::text[]
      FROM {schema}.price_atoms GROUP BY billing_class
    UNION ALL
    SELECT 'setting'::varchar(64), setting::text, NULL::text[]
      FROM {schema}.price_atoms GROUP BY setting
    UNION ALL
    SELECT 'billing_code_modifier'::varchar(64), NULL::text,
           billing_code_modifier::text[]
      FROM {schema}.price_atoms GROUP BY billing_code_modifier
    UNION ALL
    SELECT 'additional_information'::varchar(64),
           additional_information::text, NULL::text[]
      FROM {schema}.price_atoms GROUP BY additional_information
)
SELECT attr_kind,
       (row_number() OVER (
...
_PRICE_DICTIONARY_DIFFERENCE_SQL = """
SELECT count(*)
  FROM (
...
_RECONSTRUCTED_PRICE_DIFFERENCE_SQL = """
...
SELECT count(*)
  FROM (
      (SELECT * FROM reconstructed EXCEPT ALL SELECT * FROM original)
      UNION ALL
      (SELECT * FROM original EXCEPT ALL SELECT * FROM reconstructed)
  ) differences
"""


async def _seed_price_rewrite_fixture(schema: str) -> None:
    await db.execute_ddl(_PRICE_ATOM_FIXTURE_DDL.format(schema=schema))
    await db.status(_PRICE_ATOM_FIXTURE_ROWS.format(schema=schema))
    await db.status(
        f"""CREATE UNLOGGED TABLE {schema}.price_atoms_original AS
            TABLE {schema}.price_atoms"""
    )
    await db.status(_LEGACY_PRICE_DICTIONARY_SQL.format(schema=schema))


async def _assert_price_stage_persistence(
    schema: str,
    schema_name: str,
...
async def _assert_price_dictionary_equivalence(schema: str) -> None:
    difference_count = int(
        await db.scalar(
            _PRICE_DICTIONARY_DIFFERENCE_SQL.format(schema=schema)
        )
        or 0
    )
...
async def _assert_reconstructed_price_atoms(schema: str) -> None:
    difference_count = int(
        await db.scalar(
            _RECONSTRUCTED_PRICE_DIFFERENCE_SQL.format(schema=schema)
        )
        or 0
    )
    assert difference_count == 0
    lean_rows = await db.all(
        f"""SELECT price_atom_global_id_128::text, negotiated_rate
...
async def test_real_postgres_price_atom_rewrite_uses_configured_stage_durability(
    ...
    try:
        await db.execute_ddl(f"CREATE SCHEMA {schema}")
        await _seed_price_rewrite_fixture(schema)
        manifest = (
        ...
        await _assert_price_stage_persistence(
            schema,
            schema_name,
            expected_persistence,
        )
        ...
        await _assert_price_dictionary_equivalence(schema)
        await _assert_reconstructed_price_atoms(schema)
    finally:
        await db.execute_ddl(f"DROP SCHEMA IF EXISTS {schema} CASCADE")
        await db.disconnect()
