...
def _assert_receipt_columns_match_model(elements) -> None:
    ...
    migration_columns_by_name = {element.name: element for element in elements if isinstance(element, sa.Column)}
    model_columns_by_name = {
        column.name: column for column in ProviderDirectoryProfileCapacityPreflightReceipt.__table__.columns
    }
    ...
    for column_name, migration_column in migration_columns_by_name.items():
        ...
        assert getattr(migration_column.type, "length", None) == getattr(model_column.type, "length", None)
        assert getattr(migration_column.type, "timezone", None) == getattr(model_column.type, "timezone", None)
...
def test_capacity_v2_migration_precedes_the_unique_repository_head():
    ...
    assert script.get_heads() == ["20260922010000_custom_import_execution_request_identity"]
...
def test_capacity_preflight_migration_guardedly_admits_only_lease_v3(monkeypatch):
    ...
    assert migration.down_revision == ("20260810130000_provider_directory_reviewed_subset_terminal_window")
    for contract_id in (
    ...
    assert recorder.created_table[0] == ("provider_directory_profile_capacity_preflight_receipt")
