...
def test_cursor_round_trip_is_confidential_and_redacted(monkeypatch) -> None:
    ...
    assert SNAPSHOT_ID not in token
    assert ADDRESS_KEY not in token
    assert "1234567893" not in token
...
def test_cursor_keyring_is_frozen_and_blocks_secret_serialization() -> None:
    keyring = _keyring()
...
def test_cursor_rejects_authenticated_noncanonical_json() -> None:
    ...
    spaced = plaintext.replace(b'","contract"', b'", "contract"')

    for malformed_plaintext in (duplicate, spaced):
        with pytest.raises(
            cursor.BillingSearchCursorError,
            match="^billing_search_cursor_invalid$",
        ):
            _open(_authenticated_token_for_plaintext(malformed_plaintext))


def test_cursor_parse_failures_do_not_retain_authenticated_plaintext() -> None:
    sensitive_sentinel = "SENSITIVE-SELECTOR-MUST-NOT-ESCAPE"
    token = _authenticated_token_for_plaintext(sensitive_sentinel.encode("ascii"))
...
def test_cursor_rejects_tampering_and_wrong_key_without_detail(monkeypatch) -> None:
    token = _seal(monkeypatch)
    replacement = "A" if token[-1] != "A" else "B"

    for candidate_token, candidate_keyring in (
        (token[:-1] + replacement, _keyring()),
        (
            token,
            cursor.BillingSearchCursorKeyring(
                active_key_id="k1",
                keys_by_id={"k1": b"x" * 32},
            ),
        ),
    ):
        with pytest.raises(cursor.BillingSearchCursorError) as failure:
            _open(candidate_token, keyring=candidate_keyring)
        assert str(failure.value) == "billing_search_cursor_invalid"
        assert failure.value.__cause__ is None
