...
async def test_shared_block_copy_rejects_invalid_publication_contract(
    ...
    path = tmp_path / "blocks.copy"
    if case != "missing":
        path.write_bytes(b"" if case == "empty" else b"x")
    digest = hashlib.sha256(b"x").hexdigest()
    kwargs = {}
    if case == "bytes-only":
        kwargs["expected_copy_bytes"] = 1
    elif case == "digest-only":
        kwargs["expected_copy_sha256"] = digest
    elif case == "boolean-bytes":
        kwargs.update(expected_copy_bytes=True, expected_copy_sha256=digest)
    elif case == "zero-bytes":
        kwargs.update(expected_copy_bytes=0, expected_copy_sha256=digest)
    elif case == "changed-size":
        kwargs.update(expected_copy_bytes=2, expected_copy_sha256=digest)
    elif case == "invalid-digest":
        kwargs.update(expected_copy_bytes=1, expected_copy_sha256="not-a-digest")
    elif case == "reuse-without-proof":
...
async def test_shared_block_copy_requires_driver_binary_copy(tmp_path, monkeypatch):
    ...
    connection = SimpleNamespace(raw_connection=SimpleNamespace(driver_connection=object()))
