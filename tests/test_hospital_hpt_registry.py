...
_REVIEWED_LOCATOR_NAMES = {
    ...
    **dict(pair.split(":", 1) for pair in "hospital-000685:39-3714515 BMH Oktibbeha County|hospital-000825:New Orleans Hospital|hospital-000826:New Orleans Westbank Hospital|hospital-002405:Good Shepherd Rehabilitation Hospital|hospital-003109:Mesa Springs|hospital-003110:Mesa Springs Changes|hospital-006677:UCSF Parnassus".split("|")),
...
def test_checked_in_registry_has_exact_source_neutral_shape():
    ...
    assert len(registry.hospital_hpt_registry_groups()) == 6_881
    assert len({entry["hospital_id"] for entry in hospitals}) == len(hospitals)
    assert "alias_of" not in hospital_by_id["hospital-005625"]
    assert sum("locator_name" in entry for entry in hospitals) == 1_691
    assert sum("locator_mrf_url" in entry for entry in hospitals) == 658
