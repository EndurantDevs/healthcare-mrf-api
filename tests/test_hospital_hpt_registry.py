...
def test_checked_in_registry_has_exact_source_neutral_shape():
    ...
    assert len(registry.hospital_hpt_registry_groups()) == 6_915
    assert len({entry["hospital_id"] for entry in hospitals}) == len(hospitals)
    assert sum("locator_name" in entry for entry in hospitals) == 1_681
    assert sum("locator_mrf_url" in entry for entry in hospitals) == 684
