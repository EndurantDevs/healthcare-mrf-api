...
policy_block="$(sed -n '/  quality)/,/    ;;/p' scripts/ci/prepush)"

test -n "$trigger_block"
test -n "$policy_block"
printf '%s\n' "$trigger_block" | grep -Fq -- 'types: [opened, synchronize, reopened, labeled, unlabeled]'
grep -Fq -- 'run: scripts/ci/prepush quality' "$workflow_path"
grep -Fq -- "contains(github.event.pull_request.labels.*.name, 'readability-zero-growth-approved')" "$workflow_path"
