...
async def _create_dataset_tables(database: Database, schema: str) -> None:
    """Create the source, dataset, and admission-receipt fixture surface."""
...
async def _create_resource_tables(database: Database, schema: str) -> None:
    """Create resource and normalized serving-relation fixture tables."""
...
async def _create_tables(database: Database, schema: str) -> None:
    """Create the complete normalized serving-relation fixture schema."""

    await _create_dataset_tables(database, schema)
    await _create_resource_tables(database, schema)
    await database.status(
...
from tests.provider_directory_dataset_serving_relation_guard_cases import (
    test_real_postgres_accepts_zero_network_edges,
    test_real_postgres_invalid_affiliation_refs_preserve_rows,
    test_real_postgres_invalid_network_refs_preserve_rows,
    test_real_postgres_legacy_null_root_uses_import_run_id,
    test_real_postgres_serializes_same_dataset_relation_builds,
)
