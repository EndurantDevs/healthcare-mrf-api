...
from process.custom_import import CandidateRejected, CustomImportDefinition, DefinitionError
from process.custom_import import SourceSnapshotError, assemble_root_families, load_json_definition
from process.custom_import import merge_families, validate_source_snapshot_tokens
...
def test_json_and_yaml_have_one_canonical_definition_and_digest():
    ...
    assert [alias.source_label for alias in json_definition.aliases] == [
        "Provider ID", "Provider Name", "Amount", "Provider ID", "Service Code"
    ]
...
def test_definition_enforces_profile_and_projection_limits():
    ...
    raw["schema"]["root"]["fields"].extend(
        {
            "id": f"field_{number}", "slot": number, "type": "string",
            "nullable": True, "projection_slot": number - 1,
        }
...
def test_source_tokens_must_be_complete_single_and_shared(definition):
    assert validate_source_snapshot_tokens(
...
def test_invalid_children_and_duplicate_keys_reject_only_their_root_family(definition):
    ...
    assert {item.code for item in duplicate.rejections} == {"duplicate_root_key", "duplicate_child_key"}


def test_missing_nullable_field_does_not_hide_a_later_required_field():
    raw = _raw_definition()
    child_fields = raw["schema"]["children"][0]["fields"]
    raw["schema"]["children"][0]["fields"] = [child_fields[0], child_fields[2], child_fields[1]]
...
def test_upsert_retains_rejected_and_absent_families_but_snapshot_requires_scope(definition):
    ...
    assert (
        merge_families(previous_families_by_key, candidate, refresh_mode="snapshot", complete_scope=True)
    ...
    assert merge_families(previous_families_by_key, empty, refresh_mode="snapshot", complete_scope=True) == {}
