...
"""Synthetic source-pinned selector fixtures."""
...
def selector_body(selector: dict[str, object]) -> bytes:
    return json.dumps(
        {
...
def selector_access(selector: dict[str, object]):
    body = selector_body(selector)
    return _authorize(body, _headers(body))


def opaque_access(reference: str | None = None):
    return selector_access(
        {"billing_entity_ref": reference or billing_entity_reference()}
    )


def ein_access(value: str = SYNTHETIC_EIN):
    return selector_access(
        {"tax_identity": {"type": "ein", "value": value}}
    )


def npi_access():
    return selector_access(
        {"tax_identity": {"type": "npi", "value": str(NPI)}}
    )
    ...
    **overrides: object,
    ...
    fields_by_name = {
        ...
        "provider_tax_identity_source_publication": (
            publication or source_publication()
...
def source_pinned_selection(
    *,
    ...
    healthporta_plan_id: str = PLAN_ID,
    plan_release_id: str = PLAN_RELEASE_ID,
) -> PlanReleaseServingSelection:
