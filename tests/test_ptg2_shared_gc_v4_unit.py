...
def test_owned_v4_abandonment_token_is_stable_after_claim() -> None:
    claimed_token = shared_gc._owned_v4_abandonment_token(_BUILD_TOKEN)

    assert shared_gc._owned_v4_abandonment_token(claimed_token) == claimed_token
...
async def test_owned_v4_finalization_rejects_incomplete_cleanup(monkeypatch):
    ...
    final_delete_sql = failed_delete.status.await_args.args[0]
    assert "NOT EXISTS" in final_delete_sql
    assert "ptg2_block_build_pin" in final_delete_sql
