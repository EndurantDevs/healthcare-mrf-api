...
@pytest.mark.parametrize(
    "overrides",
    (
        {"lock_timeout_ms": 500, "statement_timeout_ms": 500},
        {"statement_timeout_ms": 300_000, "operation_timeout_seconds": 300.0},
        {"build_lease_seconds": 3600, "operation_timeout_seconds": 3600.0},
    ),
)
def test_limits_require_strict_timeout_ordering(overrides):
    with pytest.raises(
        TinNpiConnectorPublicationError,
        match="connector publication limits are invalid",
    ):
        replace(_limits_for(_empty_bundle().counts), **overrides)
    ...
    (
        {
            ...
            "operation_timeout_seconds": 0.003,
            "build_lease_seconds": 30,
        },
        {
            "lock_timeout_ms": 30_000,
            "statement_timeout_ms": 3_600_000,
            "operation_timeout_seconds": 3600.001,
            "build_lease_seconds": 3601,
        },
    ),
)
def test_limits_accept_strict_timeout_ordering(overrides):
    limits = replace(_limits_for(_empty_bundle().counts), **overrides)

    assert limits.lock_timeout_ms < limits.statement_timeout_ms
    assert limits.statement_timeout_ms < limits.operation_timeout_seconds * 1000
    assert limits.operation_timeout_seconds < limits.build_lease_seconds
