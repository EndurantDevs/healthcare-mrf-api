...
"""Add the closed TIGER replacement family to generation authority.

Revision ID: 20260920110000_tiger_result_generation
Revises: 20260920100000_cms_doctors_result_generation
"""
...
revision = "20260920110000_tiger_result_generation"
down_revision = "20260920100000_cms_doctors_result_generation"
branch_labels = None
depends_on = None
...
def _schema() -> str:
    ...
    if schema == "tiger":
        raise RuntimeError("TIGER authority requires a separate application schema")
    if not _IDENTIFIER.fullmatch(schema) or len(schema.encode("utf-8")) > 63:
        raise RuntimeError("TIGER result generation schema is invalid")
    return schema


def _quoted(value: str) -> str:
    if _IDENTIFIER.fullmatch(value) is None:
        raise RuntimeError("TIGER result generation identifier is invalid")
    return f'"{value}"'
...
def upgrade() -> None:
    """Admit one generation-less TIGER row without inventing legacy history."""

    schema = _schema()
    cardinality_by_importer = {"tiger": _TIGER_CARDINALITY, **_REFERENCE_CARDINALITY}
    _replace_shape_constraint(schema, cardinality_by_importer)
    table = f"{_quoted(schema)}.{_quoted(_STATE_TABLE)}"
    op.execute(
        sa.text(
            f"INSERT INTO {table} (importer_id, local_lineage_id, local_generation) VALUES ('tiger', :lineage_id, 0)"
        ).bindparams(
            sa.bindparam(
                "lineage_id",
                value=uuid4(),
...
def downgrade() -> None:
    """Refuse to erase recorded TIGER publication or adoption evidence."""
    ...
    retained = (
        ...
        .execute(
            sa.text(
                f"SELECT EXISTS (SELECT 1 FROM {table} WHERE importer_id='tiger' "
    ...
    if retained:
        raise RuntimeError("TIGER result generation evidence prevents downgrade")
    op.execute(f"DELETE FROM {table} WHERE importer_id='tiger'")
