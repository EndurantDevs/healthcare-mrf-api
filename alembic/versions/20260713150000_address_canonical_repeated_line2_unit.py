"""Reapply canonical functions for repeated bare line-two units."""
...
revision = "20260713150000_address_canonical_repeated_line2_unit"
down_revision = "20260713120000_provider_directory_fhir_completeness"
branch_labels = None
depends_on = None
...
def _repeated_line2_unit_is_current(bind, foundation, schema: str) -> bool:
        ...
        bind.exec_driver_sql(
            ...
            SELECT
                ...
                AND {qschema}.addr_identity_key_v1(
                    ...
                    '27834-2818',
                    'US'
                ) = 'v2|2100stantonsburgrdgme|ofc||NC|27834|US|street';
            """
        ).scalar()
    )
...
def upgrade():
    foundation = _migration_module(
        ...
        "_address_canonical_foundation_repeated_line2",
    )
    ...
    if not prior_replay._functions_owned_by_current_user(bind, schema):
        if _repeated_line2_unit_is_current(bind, foundation, schema):
            return
        raise RuntimeError(
            "Canonical address SQL functions need the repeated line-two unit replay, "
    ...
    if not _repeated_line2_unit_is_current(bind, foundation, schema):
        raise RuntimeError("Canonical repeated line-two unit replay did not take effect")
